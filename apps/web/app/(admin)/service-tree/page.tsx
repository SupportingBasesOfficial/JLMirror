...
export default function ServiceTreePage() {
  ...
  const tree = useMemo(() => {
    const buildTree = (
      nodes: ZabbixService[],
      depth: number = 0,
    ): Array<ZabbixService & { _depth: number }> => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      critical,
      unknown,
      total: ok + degraded + critical + unknown,
    };
  }, [services]);
              ...
              return (
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      color: COLORS.text,
                      fontWeight: node._depth === 0 ? "bold" : "normal",
                    }}
                  >
                    {node.name}
                  </span>
                  ...
                  {node.description && (
                    <span
                      className="text-[10px] truncate max-w-xs hidden md:inline"
                      style={{ color: COLORS.muted }}
                    >
                      {node.description}
                    </span>
      ...
      {!loading && !error && services.length === 0 && (
        ...
        >
          ...
          <p className="text-sm" style={{ color: COLORS.muted }}>
            Nenhum serviço configurado no Zabbix
          </p>
          <p className="text-[10px] mt-1" style={{ color: COLORS.muted }}>
            Configure IT Services no Zabbix para visualizar a árvore
          </p>
        </div>
      )}
    </div>
