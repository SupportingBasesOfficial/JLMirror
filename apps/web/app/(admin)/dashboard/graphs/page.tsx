...
import { RefreshCw, X, ChevronRight, BarChart3, Server, Clock } from "lucide-react";
...
function GraphChart({ series, graphtype }: { series: GraphSeries[]; graphtype?: number }) {
  ...
  useEffect(() => {
    ...
    if (isPie) {
      ...
      const categories = series[0]?.points.map((p) => new Date(p.clock * 1000).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })) ?? [];
      ...
      chartInstance.current.setOption({
        ...
        xAxis: { type: "category", data: categories, axisLabel: { color: "#666", fontSize: 10 } },
        yAxis: { type: "value", axisLabel: { color: "#666", fontSize: 10 } },
        series: barSeries,
        grid: { top: 30, bottom: 10, left: 50, right: 10, containLabel: true },
      }, true);
    } else {
      const lineSeries = series.map((s) => ({
        ...
        itemStyle: { color: s.color },
        areaStyle: s.drawtype === 1 ? { color: s.color, opacity: 0.15 } : undefined,
      }));
      chartInstance.current.setOption({
        tooltip: { trigger: "axis", formatter: (params: unknown) => {
          const arr = params as Array<{ axisValue: string; seriesName: string; value: [number, number] }>;
        ...
        legend: { textStyle: { color: "#999", fontSize: 11 }, top: 0, type: "scroll" },
        xAxis: { type: "time", axisLabel: { color: "#666", fontSize: 10 } },
        yAxis: { type: "value", axisLabel: { color: "#666", fontSize: 10 }, scale: true },
        series: lineSeries,
        grid: { top: 30, bottom: 10, left: 50, right: 10, containLabel: true },
      }, true);
    }
...
function GraphDetailModal({ graph, onClose }: { graph: ZabbixGraph; onClose: () => void }) {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-4 border-b" style={{ borderColor: "var(--border-default)" }}>
          <div>
            <h2 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>{graph.name}</h2>
            <div className="flex items-center gap-3 mt-1 text-[12px]" style={{ color: "var(--text-muted)" }}>
              ...
              <span>{graph.width}x{graph.height}px</span>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center gap-1 p-3 border-b" style={{ borderColor: "var(--border-default)" }}>
          {TIME_RANGES.map((tr) => (
            <button
              key={tr.label}
              ...
              style={
                timeRange === tr.seconds
                  ? { background: "var(--brand-primary)", color: "var(--surface-0)" }
        ...
        <div className="p-4">
          {error && <ErrorState title="Erro ao carregar dados" message={error} />}
          {isLoading && <LoadingState label="Buscando dados de history..." />}
          {data?.data && (
            <>
              {data.data.series.length === 0 || data.data.series.every((s) => s.points.length === 0) ? (
                <EmptyState title="Sem dados no periodo selecionado" />
              ) : (
                <GraphChart series={data.data.series} graphtype={graph.graphtype} />
              )}
              <div className="mt-4 space-y-1.5">
                <h3 className="text-[13px] font-bold" style={{ color: "var(--text-primary)" }}>Items neste grafo</h3>
                {data.data.series.map((s) => (
                  <div key={s.itemid} className="flex items-center gap-2 text-[12px]" style={{ color: "var(--text-muted)" }}>
                    <span className="w-3 h-3 rounded" style={{ background: s.color }} />
                    <span style={{ color: "var(--text-primary)" }}>{s.name}</span>
                    {s.units && <span style={{ color: "var(--text-muted)" }}>({s.units})</span>}
                    <span className="ml-auto" style={{ color: "var(--text-muted)" }}>{s.points.length} pontos</span>
                  </div>
                ))}
              </div>
            </>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--text-primary)" }}>Gráficos Zabbix</h1>
      ...
      {isLoading ? (
        ...
        <EmptyState title={search ? "Nenhum grafo encontrado para a busca" : "Nenhum grafo configurado no Zabbix"} />
      ) : (
          ...
          {filteredGraphs.map((g, idx) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <h3 className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{g.name}</h3>
              ...
              <div className="flex items-center gap-3 text-[11px] mb-2" style={{ color: "var(--text-muted)" }}>
                ...
                {g.hosts && g.hosts.length > 0 && (
                  <span className="flex items-center gap-1">
                    <Server size={11} />
                    {g.hosts[0].name}{g.hosts.length > 1 ? ` +${g.hosts.length - 1}` : ""}
                  </span>
                )}
              </div>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {g.gitems.slice(0, 5).map((gi, giIdx) => (
                    <span
                      ...
                      style={{ background: `color-mix(in srgb, ${gi.color} 20%, transparent)`, color: gi.color }}
                    >
                      {gi.itemid}
                    </span>
                  ))}
      ...
      {selectedGraph && (
        <GraphDetailModal graph={selectedGraph} onClose={() => setSelectedGraph(null)} />
      )}
    </div>
  );
}
