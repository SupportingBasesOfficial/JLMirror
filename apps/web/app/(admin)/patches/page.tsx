...
export default function PatchesPage() {
  const { data: summaryData, mutate: mutateSummary } = useApi<{
    summary: PatchSummary;
    recent_scans: unknown[];
  }>("/api/v1/patches/summary");
  const { data: patchesData, mutate: mutatePatches } = useApi<{
    patches: Patch[];
  }>("/api/v1/patches?limit=200");
  const { data: deploymentsData, mutate: mutateDeployments } = useApi<{
    deployments: Deployment[];
  }>("/api/v1/patches/deployments");
  ...
  }, [patchesData, filterSeverity, filterStatus]);

  const handleAction = async (
    patchId: string,
    action: "approve" | "reject" | "install",
  ) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold" style={{ color: COLORS.teal }}>
            <Shield size={18} className="inline mr-1" /> Patch Management
          </h1>
                ...
                >
                  ...
                  <td className="p-3">
                    <span
                      ...
                      style={{
                        background: `${STATUS_COLORS[p.status] ?? COLORS.muted}15`,
                        color: STATUS_COLORS[p.status] ?? COLORS.muted,
                      }}
                    >
                    ...
                    <div className="flex items-center justify-end gap-1">
                        ...
                        <>
                          <button
                            ...
                            style={{
                              ...
                              cursor: "pointer",
                              opacity:
                                actionLoading === `${p.id}-approve` ? 0.5 : 1,
                            }}
                          >
                          ...
                          <button
                            ...
                            style={{
                              ...
                              cursor: "pointer",
                              opacity:
                                actionLoading === `${p.id}-reject` ? 0.5 : 1,
                            }}
                          >
                      ...
                      {(p.status === "approved" || p.status === "failed") && (
                        <button
                          ...
                          disabled={actionLoading === `${p.id}-install`}
                          className="px-2 py-1 rounded text-[10px] font-bold"
                          style={{
                            background: `${COLORS.teal}15`,
                            border: `1px solid ${COLORS.teal}`,
                            ...
                            cursor: "pointer",
                            opacity:
                              actionLoading === `${p.id}-install` ? 0.5 : 1,
                          }}
                        >
                          {actionLoading === `${p.id}-install` ? (
                            <Loader2 size={10} className="animate-spin" />
                          ) : (
          ...
          {deployments.map((d) => (
            ...
            >
              <div>
                ...
                >
                  <span>
                    {d.total_patches} patches · {d.total_devices} dispositivos
                  </span>
                  {d.requires_reboot && (
                    <span style={{ color: COLORS.amber }}>
                      <AlertTriangle size={9} className="inline" /> Requer
                      reboot
                    </span>
                  )}
                  <span>
                    <Clock size={9} className="inline" />{" "}
                    {formatTime(d.scheduled_at ?? d.created_at)}
                  </span>
                </div>
              </div>
