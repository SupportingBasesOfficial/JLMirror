...
export default function AnomalyDetectionPage() {
  const { data: statsData, mutate: mutateStats } = useApi<AnomalyStats>(
    "/api/v1/anomaly/stats",
  );
  const { data: detectionsData, mutate: mutateDetections } = useApi<{
    detections: AnomalyDetection[];
  }>("/api/v1/anomaly/detections?status=open&limit=30");
  ...
  const handleAcknowledge = useCallback(
    async (id: string) => {
      await fetch(`/api/v1/anomaly/detections/${id}/acknowledge`, {
        method: "PUT",
        credentials: "include",
      });
      mutateDetections();
  ...
  const handleResolve = useCallback(
    async (id: string) => {
      await fetch(`/api/v1/anomaly/detections/${id}/resolve`, {
        method: "PUT",
        credentials: "include",
      });
      mutateDetections();
  ...
  const handleFalsePositive = useCallback(
    async (id: string) => {
      await fetch(`/api/v1/anomaly/detections/${id}/false-positive`, {
        method: "PUT",
        credentials: "include",
      });
      mutateDetections();
      ...
      >
        <div className="p-4 border-b" style={{ borderColor: COLORS.border }}>
          <h3
            ...
            style={{ color: COLORS.muted }}
          >
            Anomalias em Aberto ({detections.length})
          </h3>
        </div>
        <div className="divide-y" style={{ borderColor: COLORS.border }}>
          {detections.map((det) => {
            const sevConfig =
              SEVERITY_CONFIG[det.severity] ?? SEVERITY_CONFIG.info!;
            return (
              <div key={det.id} className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        ...
                        {det.device_hostname} · {det.algorithm} · score:{" "}
                        {det.deviation_score.toFixed(2)}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => handleAcknowledge(det.id)}
                    ...
                    <button
                      onClick={() => handleFalsePositive(det.id)}
                      className="px-2 py-1 rounded text-[10px] font-bold"
                      style={{
                        background: `${COLORS.muted}15`,
                        border: `1px solid ${COLORS.muted}`,
                ...
                >
                  <div>
                    <span style={{ color: COLORS.red }}>Observado:</span>{" "}
                    {det.observed_value.toFixed(2)}
                  </div>
                  <div>
                    <span style={{ color: COLORS.green }}>Esperado:</span>{" "}
                    {det.expected_value.toFixed(2)}
                  </div>
                  <div>
                    <span>Limite inf:</span>{" "}
                    {det.threshold_low?.toFixed(2) ?? "—"}
                  </div>
                  <div>
                    <span>Limite sup:</span>{" "}
                    {det.threshold_high?.toFixed(2) ?? "—"}
                  </div>
                </div>
                <p className="text-[10px] mt-1" style={{ color: COLORS.muted }}>
                  Detectado: {new Date(det.detected_at).toLocaleString("pt-BR")}{" "}
                  · Janela: {det.window_size} amostras
                </p>
              </div>
            );
          })}
