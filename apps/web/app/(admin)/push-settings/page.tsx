...
export default function PushSettingsPage() {
  ...
  const { data: subsData, mutate } = useApi<{
  ...
  const handleTest = async () => {
    ...
    setTestResult(
      ok ? "Push enviado! Verifique seu dispositivo." : "Erro ao enviar push.",
    );
    if (ok) mutate();
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px]" style={{ color: COLORS.muted }}>
                Seu navegador não suporta Web Push API ou Service Workers.
              </p>
            </div>
          </div>
        </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                {isSubscribed ? (
                  ...
                  <div
                    className="w-10 h-10 rounded-full flex items-center justify-center"
                    style={{ background: `${COLORS.muted}15` }}
                  >
            ...
            {testResult && (
              <div
                ...
                style={{
                  color: testResult.includes("Erro")
                    ? COLORS.red
                    : COLORS.green,
                }}
              >
            ...
            {permission === "denied" && (
              ...
              >
                Permissão de notificações foi negada. Para reativar, ajuste as
                ...
                >
                  <div className="flex items-center gap-3">
                    <Smartphone
                      size={14}
                      style={{
                        color: sub.is_active ? COLORS.green : COLORS.muted,
                      }}
                    />
                    <div>
                      <p
                      ...
                      >
                        {sub.last_used_at
                          ? `Último uso: ${new Date(sub.last_used_at).toLocaleString("pt-BR")}`
                          : "Nunca usado"}
                      </p>
                    </div>
                  </div>
