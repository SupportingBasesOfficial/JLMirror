...
export default function ExecutiveDashboardPage() {
  const {
    ...
    isLoading: loadingOv,
    mutate: mutOv,
  } = useApi<Overview>("/api/v1/dashboard/executive/overview");
  const {
    data: alerts,
    error: errAl,
    isLoading: loadingAl,
    mutate: mutAl,
  } = useApi<Alerts>("/api/v1/dashboard/executive/alerts");
  const {
    data: summary,
    error: errSu,
    isLoading: loadingSu,
    mutate: mutSu,
  } = useApi<Summary>("/api/v1/dashboard/executive/summary");

  const error = errOv ?? errAl ?? errSu ?? null;
  const loading = loadingOv && loadingAl && loadingSu;

  const uptimeColor = overview
    ? overview.availability.uptime_pct >= 99
      ? COLORS.green
      : overview.availability.uptime_pct >= 95
        ? COLORS.amber
        : COLORS.red
    : COLORS.muted;
  const complianceColor = overview
    ? overview.compliance.score >= 80
      ? COLORS.green
      : overview.compliance.score >= 60
        ? COLORS.amber
        : COLORS.red
    : COLORS.muted;
  const ticketColor = overview
    ? overview.tickets.resolution_rate >= 80
      ? COLORS.green
      : overview.tickets.resolution_rate >= 50
        ? COLORS.amber
        : COLORS.red
    : COLORS.muted;
  const backupColor = overview
    ? overview.infrastructure.backup_success_rate >= 95
      ? COLORS.green
      : overview.infrastructure.backup_success_rate >= 80
        ? COLORS.amber
        : COLORS.red
    : COLORS.muted;
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px]" style={{ color: COLORS.muted }}>
            KPIs Agregados · Disponibilidade · Segurança · Compliance ·
            Infraestrutura
          </p>
        </div>
      ...
      {overview && (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
          <KpiCard
            label="Uptime"
            value={`${overview.availability.uptime_pct}%`}
            sub={`${overview.availability.online}/${overview.availability.total_devices} online`}
            color={uptimeColor}
            icon="↑"
          />
          <KpiCard
            label="Tickets Resolvidos"
            value={`${overview.tickets.resolution_rate}%`}
            sub={`${overview.tickets.resolved}/${overview.tickets.total} total`}
            color={ticketColor}
            icon="✓"
          />
          <KpiCard
            ...
            sub={`${overview.compliance.passed}/${overview.compliance.total_controls} controles`}
            color={complianceColor}
            icon="§"
          />
          <KpiCard
            label="Backup Success"
            value={`${overview.infrastructure.backup_success_rate}%`}
            sub={`${overview.infrastructure.backups_successful}/${overview.infrastructure.backups_total} backups`}
            color={backupColor}
            icon="💾"
          />
          <KpiCard
            ...
            sub={`${overview.tickets.critical} críticos`}
            color={
              alerts && alerts.total_alerts > 0 ? COLORS.amber : COLORS.green
            }
            icon="⚠"
          />
          <KpiCard
            ...
            sub={`${overview.assets.total} total`}
            color={COLORS.blue}
            icon="▣"
          />
        </div>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-[11px] mb-1">
                    ...
                    <span style={{ color: COLORS.muted }}>
                      ...
                      {overview.availability.total_devices}
                    </span>
                  </div>
                  <ProgressBar
                    value={overview.availability.online}
                    max={overview.availability.total_devices}
                    color={COLORS.green}
                  />
                </div>
                <div>
                  <div className="flex justify-between text-[11px] mb-1">
                    ...
                    <span style={{ color: COLORS.muted }}>
                      {overview.availability.warning}
                    </span>
                  </div>
                  <ProgressBar
                    value={overview.availability.warning}
                    max={overview.availability.total_devices}
                    color={COLORS.amber}
                  />
                </div>
                <div>
                  <div className="flex justify-between text-[11px] mb-1">
                    ...
                    <span style={{ color: COLORS.muted }}>
                      {overview.availability.offline}
                    </span>
                  </div>
                  <ProgressBar
                    value={overview.availability.offline}
                    max={overview.availability.total_devices}
                    color={COLORS.red}
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-[12px]">
                <div className="p-2 rounded" style={{ background: COLORS.bg }}>
                  ...
                  <div
                    style={{
                      color:
                        overview.infrastructure.capacity_alerts > 0
                          ? COLORS.amber
                          : COLORS.green,
                    }}
                  >
                    {overview.infrastructure.capacity_alerts}
                  </div>
                </div>
                <div className="p-2 rounded" style={{ background: COLORS.bg }}>
                  ...
                  <div
                    style={{
                      color:
                        overview.infrastructure.ssl_expiring > 0
                          ? COLORS.amber
                          : COLORS.green,
                    }}
                  >
                    {overview.infrastructure.ssl_expiring}
                  </div>
                </div>
                <div className="p-2 rounded" style={{ background: COLORS.bg }}>
                  ...
                  <div
                    style={{
                      color:
                        overview.infrastructure.ssl_expired > 0
                          ? COLORS.red
                          : COLORS.green,
                    }}
                  >
                    {overview.infrastructure.ssl_expired}
                  </div>
                </div>
                <div className="p-2 rounded" style={{ background: COLORS.bg }}>
                  <div style={{ color: COLORS.muted }}>Firewall Rules</div>
                  <div style={{ color: COLORS.blue }}>
                    {overview.infrastructure.firewall_rules}
                  </div>
                </div>
                <div className="p-2 rounded" style={{ background: COLORS.bg }}>
                  ...
                  <div
                    style={{
                      color:
                        overview.platform.critical_health_checks > 0
                          ? COLORS.red
                          : COLORS.green,
                    }}
                  >
                    {overview.platform.critical_health_checks}
                  </div>
                </div>
                <div className="p-2 rounded" style={{ background: COLORS.bg }}>
                  ...
                  <div
                    style={{
                      color:
                        overview.platform.unread_notifications > 0
                          ? COLORS.amber
                          : COLORS.muted,
                    }}
                  >
                    {overview.platform.unread_notifications}
                  </div>
                </div>
              </div>
            </div>
                  ...
                  style={{
                    background:
                      alerts.total_alerts > 0
                        ? `color-mix(in srgb, var(--status-warning-text) 12%, transparent)`
                        : `color-mix(in srgb, var(--status-ok-text) 12%, transparent)`,
                    color:
                      alerts.total_alerts > 0 ? COLORS.amber : COLORS.green,
                  }}
                >
                ...
                <div className="mb-3">
                  ...
                  {alerts.critical_tickets.map((t) => (
                    <AlertRow
                      ...
                      subtitle={formatTime(t.created_at as string)}
                      badge={t.priority as string}
              ...
              {alerts.ssl_alerts.length > 0 && (
                <div className="mb-3">
                  ...
                  {alerts.ssl_alerts.map((s) => (
                    <AlertRow
                      key={s.id as string}
                      title={s.hostname as string}
                      subtitle={`Expires: ${formatTime(s.valid_to as string)}`}
                      badge={s.status as string}
                      badgeColor={
                        s.status === "expired" ? COLORS.red : COLORS.amber
                      }
                    />
                  ))}
                </div>
                ...
                <div className="mb-3">
                  ...
                  {alerts.capacity_risks.map((cap) => (
                    <AlertRow
                      ...
                      title={cap.resource_name as string}
                      subtitle={`${cap.resource_type as string} · ${formatTime(cap.generated_at as string)}`}
                      badge={cap.confidence as string}
                      badgeColor={
                        cap.confidence === "high" ? COLORS.red : COLORS.amber
                      }
                    />
                  ))}
                </div>
              ...
              {alerts.failed_backups.length > 0 && (
                <div className="mb-3">
                  ...
                  {alerts.failed_backups.map((b) => (
                    <AlertRow
                      key={b.id as string}
                      title={b.file_path as string}
                      subtitle={formatTime(b.created_at as string)}
                      badge="failed"
                      badgeColor={COLORS.red}
                    />
                  ))}
              ...
              {alerts.health_alerts.length > 0 && (
                <div className="mb-3">
                  ...
                  {alerts.health_alerts.map((h) => (
                    <AlertRow
                      key={h.id as string}
                      title={h.check_name as string}
                      subtitle={h.message as string}
                      badge={h.severity as string}
                      badgeColor={
                        h.severity === "critical" ? COLORS.red : COLORS.amber
                      }
                    />
                  ))}
                </div>
