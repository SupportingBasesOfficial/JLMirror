...
export default function PredictiveFailurePage() {
  const { data: statsData, mutate: mutateStats } = useApi<PredictionStats>(
    "/api/v1/predictions/stats",
  );
  const { data: predictionsData, mutate: mutatePredictions } = useApi<{
    predictions: FailurePrediction[];
  }>("/api/v1/predictions?status=open&limit=30");
  ...
  const handleAcknowledge = useCallback(
    async (id: string) => {
      await fetch(`/api/v1/predictions/${id}/acknowledge`, {
        method: "PUT",
        credentials: "include",
      });
      mutatePredictions();
  ...
  const handleMitigate = useCallback(
    async (id: string) => {
      await fetch(`/api/v1/predictions/${id}/mitigate`, {
        method: "PUT",
        credentials: "include",
      });
      mutatePredictions();
  ...
  const handleOccurred = useCallback(
    async (id: string) => {
      await fetch(`/api/v1/predictions/${id}/occurred`, {
        method: "PUT",
        credentials: "include",
      });
      mutatePredictions();
  ...
  const handleFalsePositive = useCallback(
    async (id: string) => {
      await fetch(`/api/v1/predictions/${id}/false-positive`, {
        method: "PUT",
        credentials: "include",
      });
      mutatePredictions();
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold" style={{ color: COLORS.teal }}>
            <TrendingDown size={18} className="inline mr-1" /> Predictive
            Failure
          </h1>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard
            ...
            color={stats.accuracy > 70 ? COLORS.green : COLORS.amber}
            icon={<Target size={14} />}
          />
        </div>
      )}
        ...
        >
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-lg font-bold" style={{ color: COLORS.green }}>
                {stats.mitigated}
              </p>
            </div>
            <div>
              ...
              <p className="text-lg font-bold" style={{ color: COLORS.muted }}>
      ...
      >
        <div className="p-4 border-b" style={{ borderColor: COLORS.border }}>
          <h3
            ...
            style={{ color: COLORS.muted }}
          >
            Predições em Aberto ({predictions.length})
          </h3>
        </div>
        <div className="divide-y" style={{ borderColor: COLORS.border }}>
          {predictions.map((pred) => {
            const sevConfig =
              SEVERITY_CONFIG[pred.severity] ?? SEVERITY_CONFIG.info!;
            ...
            return (
              <div key={pred.id} className="p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        ...
                        {pred.device_hostname} · {pred.model_type}
                      </p>
                    </div>
                  </div>
                ...
                <div
                  className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[10px]"
                  style={{ color: COLORS.muted }}
                >
                  <div>
                    <span style={{ color: COLORS.text }}>Atual:</span>{" "}
                    {pred.current_value.toFixed(2)}
                  </div>
                  <div>
                    <span style={{ color: COLORS.text }}>Predito:</span>{" "}
                    {pred.predicted_value.toFixed(2)}
                  </div>
                  <div>
                    <span style={{ color: COLORS.text }}>Threshold:</span>{" "}
                    {pred.threshold_value.toFixed(2)}
                  </div>
                  <div>
                    <span style={{ color: COLORS.text }}>Slope:</span>{" "}
                    {pred.trend_slope?.toFixed(4) ?? "—"}
                  </div>
                </div>
                {pred.estimated_failure_hours !== null && (
                  ...
                  >
                    ...
                    {pred.estimated_failure_at &&
                      ` (${new Date(pred.estimated_failure_at).toLocaleString("pt-BR")})`}
                  </div>
                )}
                {pred.r_squared !== null && (
                  <p
                    ...
                    style={{ color: COLORS.muted }}
                  >
                    R² = {pred.r_squared.toFixed(4)} · Detectado:{" "}
                    {new Date(pred.detected_at).toLocaleString("pt-BR")}
                  </p>
                )}
                <div className="flex items-center gap-2 mt-3">
                  <button
                    onClick={() => handleAcknowledge(pred.id)}
                  ...
                  <button
                    onClick={() => handleOccurred(pred.id)}
                    className="px-2 py-1 rounded text-[10px] font-bold"
                    style={{
                  ...
                  <button
                    onClick={() => handleFalsePositive(pred.id)}
                    className="px-2 py-1 rounded text-[10px] font-bold"
                    style={{
                      background: `${COLORS.muted}15`,
                      border: `1px solid ${COLORS.muted}`,
          ...
          {predictions.length === 0 && (
            <div className="p-6 text-center">
              ...
              />
              <p className="text-[12px]" style={{ color: COLORS.muted }}>
                Nenhuma predição de falha em aberto
              </p>
            </div>
          )}
        </div>
