...
export function RealtimeToasts() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[100] space-y-2 max-w-sm" style={{ fontFamily: "'JetBrains Mono','Consolas',monospace" }}>
      {!isConnected && (
        <div className="rounded-md px-3 py-2 text-[11px]" style={{ background: "#6E7F8810", border: "1px solid #6E7F8830", color: "#6E7F88" }}>
          WebSocket desconectado — tentando reconectar...
        </div>
      )}
      {notifications.slice(0, 5).map((n) => {
        const colors = SEVERITY_COLORS[n.severity ?? "info"] ?? SEVERITY_COLORS.info;
        return (
          <div
            key={n.id}
            className="rounded-md p-3 flex items-start gap-3 animate-in fade-in slide-in-from-right-5"
            style={{ background: "#10171C", border: `1px solid ${colors.border}` }}
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[12px] font-bold" style={{ color: "#C9D4DA" }}>
                  {n.title}
                </span>
              </div>
