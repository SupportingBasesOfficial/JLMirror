...
async function updateRuntimeGauges(): Promise<void> {
  ...
  try {
    ...
    for (const queueName of ["task-scheduler", "alerting-engine", "device-sync", "partition-manager", "correlation-engine"]) {
...
metricsRoute.get("/", async (c) => {
  ...
  for (const [status, count] of metrics.requestsByStatus) {
    lines.push(`jlmirror_http_requests_by_status{status="${status}"} ${count}`);
  }

  lines.push("# TYPE jlmirror_http_requests_by_method counter");
  for (const [method, count] of metrics.requestsByMethod) {
    lines.push(`jlmirror_http_requests_by_method{method="${method}"} ${count}`);
  }
