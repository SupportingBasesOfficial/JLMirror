...
settingsRoute.use("/*", jwtAuth);
settingsRoute.use("/*", tenantContext);
...
settingsRoute.get("/", requirePermission("settings:read"), async (c) => {
  ...
  if (settings.smtp_password_encrypted !== undefined) {
    settings.smtp_password_encrypted = settings.smtp_password_encrypted ? "***" : null;
  }
...
settingsRoute.put("/", requirePermission("settings:write"), async (c) => {
  ...
  const existing = await query("SELECT id FROM public.tenant_settings WHERE tenant_id = $1", [tenantId]);
  if (!existing.data?.rows[0]) {
    await query("INSERT INTO public.tenant_settings (tenant_id) VALUES ($1)", [tenantId]);
  }
  ...
  const fieldMap: Record<string, string> = {
    company_name: "company_name", logo_url: "logo_url", primary_color: "primary_color",
    secondary_color: "secondary_color", custom_css: "custom_css", login_message: "login_message",
    smtp_enabled: "smtp_enabled", smtp_host: "smtp_host", smtp_port: "smtp_port",
    smtp_username: "smtp_username", smtp_from_email: "smtp_from_email", smtp_from_name: "smtp_from_name",
    smtp_use_tls: "smtp_use_tls", smtp_use_ssl: "smtp_use_ssl",
    slack_webhook_url: "slack_webhook_url", slack_enabled: "slack_enabled",
    discord_webhook_url: "discord_webhook_url", discord_enabled: "discord_enabled",
    telegram_chat_id: "telegram_chat_id", telegram_enabled: "telegram_enabled",
    max_devices: "max_devices", max_users: "max_users", max_api_keys: "max_api_keys",
    max_webhooks: "max_webhooks", max_scheduled_tasks: "max_scheduled_tasks",
    max_storage_mb: "max_storage_mb", max_retention_days: "max_retention_days",
    enable_monitoring: "enable_monitoring", enable_alerts: "enable_alerts",
    enable_tickets: "enable_tickets", enable_kb: "enable_kb",
    enable_reports: "enable_reports", enable_api_access: "enable_api_access",
  ...
  if (data.smtp_password_encrypted !== undefined && data.smtp_password_encrypted !== "***") {
    updateFields.push(`smtp_password_encrypted = $${paramIdx++}`);
    params.push(data.smtp_password_encrypted);
  }

  if (data.telegram_bot_token !== undefined && data.telegram_bot_token !== "***") {
...
settingsRoute.post("/test-smtp", requirePermission("settings:write"), async (c) => {
  ...
  if (!data.smtp_host || port < 1 || port > 65535) {
    return c.json({ error: { code: "SMTP_CONFIG_ERROR", message: "Configuração SMTP inválida" } }, 400);
  }
  ...
  try {
    const transporter = nodemailer.createTransport({
      ...
      auth: data.smtp_username ? {
        ...
        pass: data.smtp_password_encrypted && data.smtp_password_encrypted !== "***"
...
settingsRoute.get("/usage", requirePermission("settings:read"), async (c) => {
  ...
  const deviceCount = await query("SELECT COUNT(*) as count FROM public.devices WHERE tenant_id = $1", [tenantId]);
  const userCount = await query("SELECT COUNT(*) as count FROM public.tenant_users WHERE tenant_id = $1", [tenantId]);
  const apiKeyCount = await query("SELECT COUNT(*) as count FROM public.api_keys WHERE tenant_id = $1", [tenantId]);
  const webhookCount = await query("SELECT COUNT(*) as count FROM public.webhooks WHERE tenant_id = $1", [tenantId]);
  const taskCount = await query("SELECT COUNT(*) as count FROM public.scheduled_tasks WHERE tenant_id = $1", [tenantId]);

  const getCount = (result: { data?: { rows?: Array<Record<string, unknown>> } | null }, fallback = "0"): number => {
  ...
  return c.json({
    limits,
    usage: {
      devices: { current: getCount(deviceCount), max: limits.max_devices ?? 100 },
      users: { current: getCount(userCount), max: limits.max_users ?? 50 },
      api_keys: { current: getCount(apiKeyCount), max: limits.max_api_keys ?? 20 },
      webhooks: { current: getCount(webhookCount), max: limits.max_webhooks ?? 10 },
