...
lgpdRoute.use("/*", jwtAuth);
lgpdRoute.use("/*", tenantContext);

// GET /api/v1/lgpd/requests — lista solicitações LGPD
lgpdRoute.get("/requests", requirePermission("admin:tenants:read"), async (c) => {
...
lgpdRoute.post("/requests/export", requirePermission("admin:tenants:write"), async (c) => {
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  const body = await c.req.json<{ user_id: string; reason?: string }>();

  if (!body.user_id) {
    return c.json({ error: { code: "VALIDATION_ERROR", message: "user_id é obrigatório" } }, 400);
  }
  ...
  if (!requestId) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar solicitação" } }, 500);
  }
  ...
  try {
    ...
    await query(
      ...
      [user.sub, JSON.stringify({ request_id: requestId, user_id: body.user_id })],
    );
...
lgpdRoute.post("/requests/delete", requirePermission("admin:tenants:write"), async (c) => {
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  const body = await c.req.json<{ user_id: string; reason?: string; mode?: "anonymize" | "delete" }>();
  ...
  if (body.user_id === user.sub) {
    return c.json({ error: { code: "SELF_DELETE_FORBIDDEN", message: "Não é possível deletar seus próprios dados por esta rota" } }, 403);
  }
  ...
  try {
    if (mode === "delete") {
      ...
      await query("DELETE FROM public.sessions WHERE user_id = $1", [body.user_id]);
      await query("DELETE FROM public.trusted_devices WHERE user_id = $1", [body.user_id]);
      await query("DELETE FROM public.user_profiles WHERE user_id = $1", [body.user_id]);
      await query("DELETE FROM public.user_security_log WHERE user_id = $1", [body.user_id]);
      await query("DELETE FROM public.tenant_users WHERE user_id = $1", [body.user_id]);
    ...
    await query(
      ...
      [user.sub, JSON.stringify({ request_id: requestId, user_id: body.user_id, mode })],
    );
    ...
    return c.json({
      ...
      status: "completed",
      mode,
      message: mode === "delete" ? "Dados pessoais deletados permanentemente" : "Dados pessoais anonimizados",
    });
  } catch (err) {
    const errorMsg = err instanceof Error ? err.message : "Erro desconhecido";
    await query(
    ...
    return c.json({ error: { code: "DELETE_ERROR", message: errorMsg } }, 500);
  }
});

// GET /api/v1/lgpd/requests/:id — detalhes de uma solicitação
lgpdRoute.get("/requests/:id", requirePermission("admin:tenants:read"), async (c) => {
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  ...
  if (!result.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Solicitação não encontrada" } }, 404);
  }
