...
assetRoute.post("/", requirePermission("assets:write"), async (c) => {
  const user = c.get("user");
...
assetRoute.put("/:id", requirePermission("assets:write"), async (c) => {
  const assetId = c.req.param("id");
  const user = c.get("user");
...
assetRoute.delete("/:id", requirePermission("assets:write"), async (c) => {
  const assetId = c.req.param("id");
  const user = c.get("user");

  await query("DELETE FROM public.assets WHERE id = $1 AND tenant_id = $2", [
    assetId,
    user?.tenant_id ?? null,
  ]);
...
assetRoute.delete(
  ...
  async (c) => {
    ...
    await query(
      "SELECT public.log_asset_change($1, 'license_removed', 'license', NULL, NULL, $2)",
      [assetId, user.sub],
    );
