...
correlationRoute.use("/*", jwtAuth);
correlationRoute.use("/*", tenantContext);

const groupingStrategySchema = z.enum(["same_device", "same_host_group", "same_tag", "same_severity", "cross_device"]);
...
correlationRoute.post("/rules", requirePermission("health:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    ...
    [
      ...
      data.time_window_seconds, data.grouping_strategy, data.tag_key ?? null,
      data.min_severity, data.escalation_threshold, data.escalated_severity,
      data.suppress_individual, data.auto_create_incident, data.send_group_notification,
      data.group_channel_ids, data.is_active, user.sub,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar regra de correlação" } }, 500);
  }
...
correlationRoute.put("/rules/:id", requirePermission("health:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    name: "name", description: "description", time_window_seconds: "time_window_seconds",
    grouping_strategy: "grouping_strategy", tag_key: "tag_key", min_severity: "min_severity",
    escalation_threshold: "escalation_threshold", escalated_severity: "escalated_severity",
    suppress_individual: "suppress_individual", auto_create_incident: "auto_create_incident",
    send_group_notification: "send_group_notification", is_active: "is_active",
  };
...
correlationRoute.delete("/rules/:id", requirePermission("health:write"), async (c) => {
  const ruleId = c.req.param("id");
  const user = c.get("user");
...
correlationRoute.get("/groups", requirePermission("health:read"), httpCache(15), async (c) => {
  const user = c.get("user");
  ...
  if (status) { conditions.push(`g.status = $${paramIdx++}`); params.push(status); }
  if (severity) { conditions.push(`g.severity = $${paramIdx++}`); params.push(severity); }
  params.push(limit);
...
correlationRoute.get("/groups/:id", requirePermission("health:read"), async (c) => {
  ...
  if (!groupResult.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Grupo não encontrado" } }, 404);
  }
...
correlationRoute.post("/groups/:id/acknowledge", requirePermission("health:write"), async (c) => {
  const groupId = c.req.param("id");
  const user = c.get("user");
  ...
  if (!result.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Grupo não encontrado ou não está aberto" } }, 404);
  }
...
correlationRoute.post("/groups/:id/resolve", requirePermission("health:write"), async (c) => {
  const groupId = c.req.param("id");
  const user = c.get("user");
  ...
  if (!result.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Grupo não encontrado ou já resolvido" } }, 404);
  }
  ...
  await query(
    ...
    [user.sub, groupId, JSON.stringify({ title: result.data.rows[0].title, notes: body.notes })],
  );
...
correlationRoute.post("/groups/:id/suppress", requirePermission("health:write"), async (c) => {
  const groupId = c.req.param("id");
  const user = c.get("user");
...
correlationRoute.get("/stats", requirePermission("health:read"), httpCache(30), async (c) => {
  const user = c.get("user");
  ...
  const groupNotifications = parseInt(reduction?.group_notifications ?? "0", 10);
  const alertReductionPct = totalEvents > 0
