...
mfaRoute.post("/setup", jwtAuth, tenantContext, async (c) => {
  ...
  if (existingResult.data?.rows[0]?.is_enabled) {
    return c.json(
      { error: { code: "MFA_ALREADY_ENABLED", message: "MFA já está habilitado. Desative antes de reconfigurar." } },
      409,
    );
  }
...
mfaRoute.post("/setup/verify", jwtAuth, tenantContext, async (c) => {
  ...
  if (result.error || !result.data?.rows[0]) {
    return c.json(
      { error: { code: "MFA_NOT_SETUP", message: "MFA não foi iniciado. Chame /mfa/setup primeiro." } },
      404,
    );
  }
...
mfaRoute.post("/verify", async (c) => {
  ...
  if (challengeResult.error || !challengeResult.data?.rows[0]) {
    return c.json(
      { error: { code: "INVALID_CHALLENGE", message: "Challenge inválido ou expirado" } },
      401,
    );
  }
  ...
  if (challenge.consumed) {
    return c.json(
      { error: { code: "CHALLENGE_CONSUMED", message: "Challenge já utilizado" } },
      401,
    );
  }
  ...
  if (totpResult.error || !totpResult.data?.rows[0]) {
    return c.json(
      { error: { code: "MFA_NOT_ENABLED", message: "MFA não habilitado para este usuário" } },
      404,
    );
  }
...
mfaRoute.post("/disable", jwtAuth, tenantContext, async (c) => {
  ...
  if (!body.code || !/^\d{6}$/.test(body.code)) {
    return c.json(
      { error: { code: "VALIDATION_ERROR", message: "Código TOTP de 6 dígitos é obrigatório" } },
      400,
    );
  }
  ...
  if (result.error || !result.data?.rows[0]) {
    return c.json(
      { error: { code: "MFA_NOT_ENABLED", message: "MFA não está habilitado" } },
      404,
    );
  }
  ...
  await query(
    "DELETE FROM public.user_mfa_totp WHERE user_id = $1",
    [user.sub],
  );
...
mfaRoute.get("/status", jwtAuth, tenantContext, async (c) => {
  ...
  return c.json({
    ...
    webauthn_enabled: parseInt(webauthnResult.data?.rows[0]?.count ?? "0", 10) > 0,
    webauthn_credentials: parseInt(webauthnResult.data?.rows[0]?.count ?? "0", 10),
  });
});
