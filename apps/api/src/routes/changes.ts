...
changesRoute.use("/*", jwtAuth);
changesRoute.use("/*", tenantContext);
...
changesRoute.get("/:changeId", requirePermission("changes:read"), async (c) => {
  ...
  if (!result.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Change request não encontrado" } }, 404);
  }
...
changesRoute.post("/", requirePermission("changes:write"), async (c) => {
  ...
  // Gera RFC number
  const seqResult = await query("SELECT nextval('public.change_rfc_seq') as val");
  const seqVal = safeCount({ data: { rows: [{ count: String(seqResult.data?.rows[0]?.val ?? 1) }] } });
  ...
  const result = await query(
    ...
    [tenantId, rfcNumber, data.title, data.description ?? null,
     data.change_type, data.priority, data.risk_level,
     user.sub, data.planned_start_at ?? null, data.planned_end_at ?? null,
     JSON.stringify(data.affected_systems), JSON.stringify(data.affected_services),
     data.impact_assessment ?? null, data.rollback_plan ?? null,
     data.approval_required, data.related_ticket_id ?? null],
  );
  ...
  await query(
    ...
    [user.sub, JSON.stringify({ id: changeId, rfc: createdRfc, title: data.title })],
  );
...
changesRoute.put("/:changeId", requirePermission("changes:write"), async (c) => {
  const changeId = c.req.param("changeId");
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  ...
  const fieldMap: Record<string, string> = {
    title: "title", description: "description", change_type: "change_type",
    priority: "priority", risk_level: "risk_level", status: "status",
    assigned_to: "assigned_to", planned_start_at: "planned_start_at",
    planned_end_at: "planned_end_at", impact_assessment: "impact_assessment",
    rollback_plan: "rollback_plan", rollback_status: "rollback_status",
...
changesRoute.post("/:changeId/approve", requirePermission("changes:approve"), async (c) => {
  const changeId = c.req.param("changeId");
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  ...
  const current = currentResult.data?.rows[0] as Record<string, unknown> | undefined;
  if (!current) {
    return c.json({ error: { code: "NOT_FOUND", message: "Change request não encontrado" } }, 404);
  }
  if (current.status !== "submitted" && current.status !== "under_review") {
    return c.json({ error: { code: "INVALID_STATUS", message: "Change não está aguardando aprovação" } }, 400);
  }
  ...
  await query(
    ...
    [tenantId, changeId, user.sub, data.approver_role ?? null, data.comment ?? null],
  );
...
changesRoute.post("/:changeId/reject", requirePermission("changes:approve"), async (c) => {
  const changeId = c.req.param("changeId");
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  const body = await c.req.json<ApproveChangeInput>();
...
changesRoute.post("/:changeId/implement", requirePermission("changes:write"), async (c) => {
  const changeId = c.req.param("changeId");
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  ...
  if (current.status !== "approved" && current.status !== "scheduled") {
    return c.json({ error: { code: "INVALID_STATUS", message: "Change deve estar aprovada ou agendada" } }, 400);
  }
...
changesRoute.post("/:changeId/complete", requirePermission("changes:write"), async (c) => {
  const changeId = c.req.param("changeId");
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
...
changesRoute.post("/:changeId/rollback", requirePermission("changes:write"), async (c) => {
  const changeId = c.req.param("changeId");
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
...
changesRoute.get("/:changeId/tasks", requirePermission("changes:read"), async (c) => {
  const changeId = c.req.param("changeId");
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
...
changesRoute.post("/:changeId/tasks", requirePermission("changes:write"), async (c) => {
  const changeId = c.req.param("changeId");
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  ...
  const result = await query(
    ...
    [tenantId, changeId, data.title, data.description ?? null,
     data.task_order, data.task_type, data.assigned_to ?? null],
  );

  return c.json({ id: result.data?.rows[0]?.id, created: true });
});

changesRoute.put("/:changeId/tasks/:taskId", requirePermission("changes:write"), async (c) => {
...
changesRoute.get("/calendar/month", requirePermission("changes:read"), async (c) => {
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
...
changesRoute.get("/stats/overview", requirePermission("changes:read"), async (c) => {
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;

  const totalChanges = safeCount(await query("SELECT COUNT(*) as count FROM public.change_requests WHERE tenant_id = $1", [tenantId]));
  const pendingApproval = safeCount(await query("SELECT COUNT(*) as count FROM public.change_requests WHERE tenant_id = $1 AND status IN ('submitted','under_review')", [tenantId]));
  const approved = safeCount(await query("SELECT COUNT(*) as count FROM public.change_requests WHERE tenant_id = $1 AND status = 'approved'", [tenantId]));
  const inProgress = safeCount(await query("SELECT COUNT(*) as count FROM public.change_requests WHERE tenant_id = $1 AND status = 'in_progress'", [tenantId]));
  const implemented = safeCount(await query("SELECT COUNT(*) as count FROM public.change_requests WHERE tenant_id = $1 AND status = 'implemented'", [tenantId]));
  const failed = safeCount(await query("SELECT COUNT(*) as count FROM public.change_requests WHERE tenant_id = $1 AND status IN ('failed','rolled_back')", [tenantId]));
  const emergency = safeCount(await query("SELECT COUNT(*) as count FROM public.change_requests WHERE tenant_id = $1 AND change_type = 'emergency'", [tenantId]));

  // By type
  const byType = safeRows(await query(
  ...
  // Upcoming scheduled
  const upcoming = safeRows(await query(
  ...
  return c.json({
    ...
    failed,
    emergency,
    success_rate: totalChanges > 0 ? Math.round((implemented / totalChanges) * 100) : 0,
    by_type: byType,
    recent,
    upcoming,
  });
