  ...
  try {
    ...
    const result = await query(
      ...
      [
        ...
        operation ?? null,
        status ?? null,
        minDuration ? Number.parseInt(minDuration, 10) || null : null,
        maxDuration ? Number.parseInt(maxDuration, 10) || null : null,
        from ?? null,
        to ?? null,
        limit,
        offset,
    ...
    logger.error("Erro ao buscar traces", {
      tenantId,
...
tracesRoute.get("/stats", requirePermission("logs:read"), async (c) => {
  ...
  try {
    ...
    logger.error("Erro ao buscar stats de traces", {
      tenantId,
      error: error instanceof Error ? error.message : String(error),
    });
    return c.json(
...
tracesRoute.get("/:traceId", requirePermission("logs:read"), async (c) => {
  ...
  try {
    ...
    logger.error("Erro ao buscar detalhe do trace", {
      traceId,
      tenantId,
      error: error instanceof Error ? error.message : String(error),
    });
