...
notificationRoute.get("/channels", jwtAuth, tenantContext, requirePermission("notifications:read"), async (c) => {
  ...
  if (result.error) {
    return c.json({ error: { code: "QUERY_ERROR", message: "Erro ao buscar canais" } }, 500);
  }

  return c.json({ channels: result.data?.rows ?? [] });
});

notificationRoute.post("/channels", jwtAuth, tenantContext, requirePermission("notifications:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    `INSERT INTO public.notification_channels (tenant_id, name, channel_type, config, is_active, created_by)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id`,
    [user?.tenant_id ?? null, data.name, data.channel_type, JSON.stringify(data.config), data.is_active, user.sub],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar canal" } }, 500);
  }

  await query(
    "SELECT public.write_audit_log($1, NULL, 'notif.channel.create', 'notification_channel', $2, $3, NULL, NULL)",
    [user.sub, result.data.rows[0].id, JSON.stringify({ name: data.name, type: data.channel_type })],
  );

  return c.json({ id: result.data.rows[0].id }, 201);
});

notificationRoute.put("/channels/:id", jwtAuth, tenantContext, requirePermission("notifications:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    name: "name", channel_type: "channel_type", config: "config", is_active: "is_active", is_verified: "is_verified",
  };
...
notificationRoute.delete("/channels/:id", jwtAuth, tenantContext, requirePermission("notifications:write"), async (c) => {
  const channelId = c.req.param("id");
  const user = c.get("user");
...
notificationRoute.post("/channels/:id/test", jwtAuth, tenantContext, requirePermission("notifications:write"), async (c) => {
  const channelId = c.req.param("id");
  const user = c.get("user");

  const channelResult = await query<{ id: string; name: string; channel_type: string; config: Record<string, unknown> }>(
  ...
  if (channelResult.error || !channelResult.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Canal não encontrado" } }, 404);
  }
  ...
  await query(
    ...
    [
      user?.tenant_id ?? null, channelId,
      `[TESTE] Canal ${channel.name}`,
      `Teste de conectividade para canal ${channel.name} (${channel.channel_type})`,
      success ? "sent" : "failed", durationMs,
    ],
  );

  await query(
    "SELECT public.write_audit_log($1, NULL, 'notif.channel.test', 'notification_channel', $2, $3, NULL, NULL)",
    [user.sub, channelId, JSON.stringify({ success, duration_ms: durationMs, status_code: deliveryResult.statusCode, error: deliveryResult.error })],
  );
  ...
  return c.json({
    ...
    message: success ? "Canal testado com sucesso" : `Falha no teste: ${deliveryResult.error}`,
  });
});

// ========== Rules ==========

notificationRoute.get("/rules", jwtAuth, tenantContext, requirePermission("notifications:read"), async (c) => {
  const user = c.get("user");
  const result = await query(
...
notificationRoute.post("/rules", jwtAuth, tenantContext, requirePermission("notifications:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    ...
    [
      ...
      data.event_source, data.event_category, data.severity_filter,
      data.channel_ids, data.template_subject ?? null, data.template_body ?? null,
      data.cooldown_minutes, data.is_active, user.sub,
    ],
  );
  ...
  await query(
    ...
    [user.sub, result.data.rows[0].id, JSON.stringify({ name: data.name, source: data.event_source, channels: data.channel_ids.length })],
  );

  return c.json({ id: result.data.rows[0].id }, 201);
});

notificationRoute.put("/rules/:id", jwtAuth, tenantContext, requirePermission("notifications:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    name: "name", description: "description", event_source: "event_source", event_category: "event_category",
    severity_filter: "severity_filter", channel_ids: "channel_ids", template_subject: "template_subject",
    template_body: "template_body", cooldown_minutes: "cooldown_minutes", is_active: "is_active",
  };
...
notificationRoute.delete("/rules/:id", jwtAuth, tenantContext, requirePermission("notifications:write"), async (c) => {
  const ruleId = c.req.param("id");
  const user = c.get("user");
...
notificationRoute.post("/send", jwtAuth, tenantContext, requirePermission("notifications:write"), async (c) => {
  const user = c.get("user");
  ...
  const rulesResult = await query<{ id: string; channel_ids: string[]; cooldown_minutes: number; template_subject: string | null; template_body: string | null }>(
  ...
  const results: { rule_id: string; channel_id: string; status: string; error: string | null }[] = [];

  for (const rule of rules) {
    // Verifica cooldown
    const cooldownOk = await query<{ check_rule_cooldown: boolean }>("SELECT public.check_rule_cooldown($1) as check_rule_cooldown", [rule.id]);
    if (!cooldownOk.data?.rows[0]?.check_rule_cooldown) {
      results.push({ rule_id: rule.id, channel_id: "—", status: "rate_limited", error: "Cooldown ativo" });
      continue;
    }
    ...
    for (const channelId of rule.channel_ids) {
      const channelResult = await query<{ id: string; name: string; channel_type: string; is_active: boolean; config: Record<string, unknown> }>(
      ...
      if (!channelResult.data?.rows[0]) {
        results.push({ rule_id: rule.id, channel_id: channelId, status: "failed", error: "Canal não encontrado ou inativo" });
        continue;
      }
      ...
      await query(
        ...
        [
          user?.tenant_id ?? null, rule.id, channelId,
          data.event_source, data.event_category, data.severity,
          subject, bodyText, JSON.stringify(data.payload ?? {}),
          success ? "sent" : "failed", durationMs,
        ],
      );
      ...
      results.push({ rule_id: rule.id, channel_id: channelId, status: success ? "sent" : "failed", error: success ? null : deliveryResult.error });
    }
  }

  await query(
    "SELECT public.write_audit_log($1, NULL, 'notif.send', 'notification_log', NULL, $2, NULL, NULL)",
    [user.sub, JSON.stringify({ event_source: data.event_source, rules_matched: rules.length, results })],
  );
...
notificationRoute.get("/log", jwtAuth, tenantContext, requirePermission("notifications:read"), async (c) => {
  const user = c.get("user");
...
notificationRoute.get("/stats", jwtAuth, tenantContext, requirePermission("notifications:read"), async (c) => {
  const user = c.get("user");
  ...
  return c.json({
    channels: channelsResult.data?.rows[0] ?? { total: "0", active: "0", verified: "0" },
    rules: rulesResult.data?.rows[0] ?? { total: "0", active: "0", total_triggers: "0" },
    log_7d: logResult.data?.rows[0] ?? { total: "0", sent: "0", failed: "0", rate_limited: "0" },
