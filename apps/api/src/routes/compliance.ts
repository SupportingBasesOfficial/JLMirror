...
complianceRoute.get("/policies", jwtAuth, tenantContext, requirePermission("compliance:read"), async (c) => {
  ...
  if (framework) { conditions.push(`framework = $${paramIdx++}`); params.push(framework); }
  if (category) { conditions.push(`policy_category = $${paramIdx++}`); params.push(category); }
  ...
  if (result.error) {
    return c.json({ error: { code: "QUERY_ERROR", message: "Erro ao buscar políticas" } }, 500);
  }

  return c.json({ policies: result.data?.rows ?? [] });
});

complianceRoute.post("/policies", jwtAuth, tenantContext, requirePermission("compliance:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    `INSERT INTO public.compliance_policies (tenant_id, name, description, framework, policy_category, severity, rule_type, rule_config, check_interval_hours, is_active, created_by)
     ...
     RETURNING id`,
    [
      user?.tenant_id ?? null, data.name, data.description ?? null, data.framework,
      data.policy_category, data.severity, data.rule_type, JSON.stringify(data.rule_config),
      data.check_interval_hours, data.is_active, user.sub,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar política" } }, 500);
  }

  await query(
    "SELECT public.write_audit_log($1, NULL, 'compliance.policy.create', 'compliance_policy', $2, $3, NULL, NULL)",
    [user.sub, result.data.rows[0].id, JSON.stringify({ name: data.name, framework: data.framework })],
  );

  return c.json({ id: result.data.rows[0].id }, 201);
});

complianceRoute.put("/policies/:id", jwtAuth, tenantContext, requirePermission("compliance:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    name: "name", description: "description", framework: "framework",
    policy_category: "policy_category", severity: "severity", rule_type: "rule_type",
    check_interval_hours: "check_interval_hours", is_active: "is_active",
  };
...
complianceRoute.delete("/policies/:id", jwtAuth, tenantContext, requirePermission("compliance:write"), async (c) => {
  const policyId = c.req.param("id");
  const user = c.get("user");
...
complianceRoute.get("/scans", jwtAuth, tenantContext, requirePermission("compliance:read"), async (c) => {
  const user = c.get("user");
  ...
  if (policyId) { conditions.push(`policy_id = $${paramIdx++}`); params.push(policyId); }
...
complianceRoute.post("/scans", jwtAuth, tenantContext, requirePermission("compliance:write"), async (c) => {
  const user = c.get("user");
  const body = await c.req.json<{ policy_id: string }>();

  if (!body.policy_id) {
    return c.json({ error: { code: "VALIDATION_ERROR", message: "policy_id é obrigatório" } }, 400);
  }
  ...
  if (policyResult.error || !policyResult.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Política não encontrada ou inativa" } }, 404);
  }

  const policy = policyResult.data.rows[0] as {
    id: string; name: string; framework: string; severity: string;
    rule_type: string; rule_config: Record<string, unknown>;
  };
  ...
  if (scanResult.error || !scanResult.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao iniciar scan" } }, 500);
  }
  ...
  const checks: { name: string; description: string; passed: boolean; remediation: string | null }[] = [];
  ...
  if (ruleConfig.require_mfa === true) {
    ...
    const usersWithoutMfa = parseInt(mfaResult.data?.rows[0]?.count ?? "0", 10);
    checks.push({
      name: "MFA enabled for all users",
      description: `Encontrados ${usersWithoutMfa} usuario(s) sem MFA`,
      passed: usersWithoutMfa === 0,
      remediation: usersWithoutMfa > 0 ? "Habilite MFA para todos os usuarios ativos" : null,
    });
  }
  ...
  if (ruleConfig.require_ssl_valid === true) {
    ...
    checks.push({
      ...
      passed: expiredSsl === 0,
      remediation: expiredSsl > 0 ? "Renove os certificados SSL expirados ou proximos da expiracao" : null,
    });
  }
  ...
  if (ruleConfig.require_firewall_active === true) {
    ...
    checks.push({
      ...
      remediation: activeRules === 0 ? "Configure pelo menos uma regra de firewall ativa" : null,
    });
  }
  ...
  if (ruleConfig.require_recent_backup === true) {
    ...
    const recentBackups = parseInt(backupResult.data?.rows[0]?.count ?? "0", 10);
    checks.push({
      name: "Recent backup exists",
      description: `${recentBackups} backup(s) concluido(s) nos ultimos 7 dias`,
      passed: recentBackups > 0,
      remediation: recentBackups === 0 ? "Execute um backup completo do sistema" : null,
    });
  }
  ...
  const recentAuditLogs = parseInt(auditResult.data?.rows[0]?.count ?? "0", 10);
  checks.push({
    name: "Audit logging active",
    description: `${recentAuditLogs} log(s) de auditoria nas ultimas 24h`,
    passed: recentAuditLogs > 0,
    remediation: recentAuditLogs === 0 ? "Verifique se o sistema de auditoria esta funcionando" : null,
  });
  ...
  await query(
    `UPDATE public.compliance_scans
     ...
     WHERE id = $6`,
    [totalChecks, passedChecks, failedChecks, complianceScore, durationMs, scanId],
  );
  ...
  for (const check of checks) {
    if (!check.passed) {
      await query(
        ...
        [
          user?.tenant_id ?? null, scanId, body.policy_id,
          check.name,
          check.description,
          policy.severity,
          check.remediation ?? "Revise a configuracao e aplique as correcoes necessarias.",
        ],
      );
    }
  }
  ...
  await query(
    ...
    [user.sub, scanId, JSON.stringify({ policy: policy.name, framework: policy.framework, score: complianceScore })],
  );
...
complianceRoute.get("/violations", jwtAuth, tenantContext, requirePermission("compliance:read"), async (c) => {
  const user = c.get("user");
  ...
  if (status) { conditions.push(`v.status = $${paramIdx++}`); params.push(status); }
  if (severity) { conditions.push(`v.severity = $${paramIdx++}`); params.push(severity); }
  if (policyId) { conditions.push(`v.policy_id = $${paramIdx++}`); params.push(policyId); }
...
complianceRoute.put("/violations/:id", jwtAuth, tenantContext, requirePermission("compliance:write"), async (c) => {
  const violationId = c.req.param("id");
  const user = c.get("user");
  const body = await c.req.json<{ status: string }>();

  const validStatuses = ["open", "acknowledged", "remediated", "false_positive", "wont_fix"];
  ...
  if (body.status === "acknowledged") {
    updateFields.push(`acknowledged_by = $${paramIdx++}`, `acknowledged_at = timezone('utc'::text, now())`);
    params.push(user.sub);
  } else if (body.status === "remediated") {
    updateFields.push(`remediated_by = $${paramIdx++}`, `remediated_at = timezone('utc'::text, now())`);
    params.push(user.sub);
  }
...
complianceRoute.get("/stats", jwtAuth, tenantContext, requirePermission("compliance:read"), async (c) => {
  const user = c.get("user");
  ...
  return c.json({
    overview: overviewResult.data?.rows[0] ?? { active_policies: "0", total_policies: "0", frameworks_covered: "0" },
    violations: violationsResult.data?.rows[0] ?? { open_violations: "0", critical_open: "0", high_open: "0", remediated: "0", total_violations: "0" },
    by_framework: frameworkResult.data?.rows ?? [],
    scans: recentScansResult.data?.rows[0] ?? { total_scans: "0", avg_score: null, best_score: null, worst_score: null },
  });
});
