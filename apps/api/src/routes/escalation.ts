...
escalationRoute.use("/*", jwtAuth);
escalationRoute.use("/*", tenantContext);

const createPolicySchema = z.object({
  name: z.string().min(1).max(100),
  description: z.string().max(500).optional(),
  event_source: z.string().min(1).max(100),
  severity_filter: z.enum(["all", "info", "warning", "critical"]).default("critical"),
...
escalationRoute.get("/policies", requirePermission("notifications:read"), async (c) => {
...
escalationRoute.post("/policies", requirePermission("notifications:write"), async (c) => {
  const user = c.get("user");
  ...
  const policyResult = await query<{ id: string }>(
    ...
    [user?.tenant_id ?? null, data.name, data.description ?? null, data.event_source, data.severity_filter, data.repeat_count, data.repeat_interval_minutes, data.is_active, user.sub],
  );
  ...
  for (const step of data.steps) {
    await query(
      `INSERT INTO public.alert_escalation_steps (policy_id, tier, delay_minutes, channel_ids, template_subject, template_body)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [policyId, step.tier, step.delay_minutes, step.channel_ids, step.template_subject ?? null, step.template_body ?? null],
    );
  }

  await query(
    "SELECT public.write_audit_log($1, NULL, 'escalation.policy.create', 'alert_escalation_policies', NULL, $2, NULL, NULL)",
    [user.sub, JSON.stringify({ id: policyId, name: data.name, steps: data.steps.length })],
  );
...
escalationRoute.put("/policies/:id", requirePermission("notifications:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    name: "name", description: "description", event_source: "event_source",
    severity_filter: "severity_filter", repeat_count: "repeat_count",
    repeat_interval_minutes: "repeat_interval_minutes", is_active: "is_active",
  };
  ...
  if (data.steps) {
    await query("DELETE FROM public.alert_escalation_steps WHERE policy_id = $1", [policyId]);
    for (const step of data.steps) {
      await query(
        `INSERT INTO public.alert_escalation_steps (policy_id, tier, delay_minutes, channel_ids, template_subject, template_body)
         VALUES ($1, $2, $3, $4, $5, $6)`,
...
escalationRoute.delete("/policies/:id", requirePermission("notifications:write"), async (c) => {
  const policyId = c.req.param("id");
  const user = c.get("user");

  await query("DELETE FROM public.alert_escalation_policies WHERE id = $1 AND tenant_id = $2", [policyId, user?.tenant_id ?? null]);

  await query(
    "SELECT public.write_audit_log($1, NULL, 'escalation.policy.delete', 'alert_escalation_policies', $2, NULL, NULL, NULL)",
    [user.sub, policyId],
  );
...
escalationRoute.post("/trigger", requirePermission("notifications:write"), async (c) => {
  const user = c.get("user");
  const body = await c.req.json<{ subject: string; body: string; severity: string; source: string; payload?: Record<string, unknown> }>();

  if (!body.subject || !body.body || !body.source) {
    return c.json({ error: { code: "VALIDATION_ERROR", message: "subject, body e source são obrigatórios" } }, 400);
  }

  const tenantId = user?.tenant_id ?? null;

  // Busca política ativa para o event_source
  const policyResult = await query<{ id: string; repeat_count: number; repeat_interval_minutes: number }>(
  ...
  if (!policyResult.data?.rows[0]) {
    return c.json({ error: { code: "NO_POLICY", message: "Nenhuma política de escalonamento ativa para este evento" } }, 404);
  }

  const policy = policyResult.data.rows[0];

  // Busca step do tier 1
  const stepResult = await query<{ tier: number; delay_minutes: number; channel_ids: string[]; template_subject: string | null; template_body: string | null }>(
    "SELECT * FROM public.alert_escalation_steps WHERE policy_id = $1 ORDER BY tier ASC LIMIT 1",
    [policy.id],
  );

  if (!stepResult.data?.rows[0]) {
    return c.json({ error: { code: "NO_STEPS", message: "Política não tem steps configurados" } }, 400);
  }
  ...
  const instanceResult = await query<{ id: string }>(
    `INSERT INTO public.alert_escalation_instances (tenant_id, policy_id, alert_subject, alert_body, alert_severity, alert_source, alert_payload, current_tier, status, next_escalation_at)
     ...
     RETURNING id`,
    [tenantId, policy.id, body.subject, body.body, body.severity ?? "critical", body.source, JSON.stringify(body.payload ?? {}), String(step.delay_minutes)],
  );
  ...
  for (const channelId of step.channel_ids) {
    const channelResult = await query<{ channel_type: string; config: Record<string, unknown> }>(
    ...
    if (channelResult.data?.rows[0]) {
      ...
      await deliverNotification(channel.channel_type, channel.config ?? {}, subject, messageBody);
      ...
      await query(
        ...
        [tenantId, channelId, body.source, body.severity ?? "critical", subject, messageBody],
      );
    }
  }

  await query(
    "SELECT public.write_audit_log($1, NULL, 'escalation.trigger', 'alert_escalation_instances', NULL, $2, NULL, NULL)",
    [user.sub, JSON.stringify({ instance_id: instanceId, policy_id: policy.id, tier: 1, source: body.source })],
  );

  return c.json({ instance_id: instanceId, tier: 1, status: "active" }, 201);
});

// POST /api/v1/escalation/instances/:id/resolve — resolve um alerta escalonado
escalationRoute.post("/instances/:id/resolve", requirePermission("notifications:write"), async (c) => {
  ...
  await query(
    ...
    [user.sub, instanceId, user?.tenant_id ?? null],
  );

  await query(
    "SELECT public.write_audit_log($1, NULL, 'escalation.resolve', 'alert_escalation_instances', $2, $3, NULL, NULL)",
    [user.sub, instanceId, JSON.stringify({ resolved_by: user.sub })],
  );
...
escalationRoute.get("/instances", requirePermission("notifications:read"), async (c) => {
  const user = c.get("user");
