...
ticketRoute.get("/categories", jwtAuth, tenantContext, requirePermission("tickets:read"), async (c) => {
...
ticketRoute.post("/categories", jwtAuth, tenantContext, requirePermission("tickets:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    ...
    [user?.tenant_id ?? null, data.name, data.description ?? null, data.color, data.sla_response_hours, data.sla_resolution_hours, data.is_active],
  );
...
ticketRoute.put("/categories/:id", jwtAuth, tenantContext, requirePermission("tickets:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    ...
    sla_response_hours: "sla_response_hours", sla_resolution_hours: "sla_resolution_hours", is_active: "is_active",
  };
...
ticketRoute.delete("/categories/:id", jwtAuth, tenantContext, requirePermission("tickets:write"), async (c) => {
  const categoryId = c.req.param("id");
  const user = c.get("user");
...
ticketRoute.get("/", jwtAuth, tenantContext, requirePermission("tickets:read"), async (c) => {
  const user = c.get("user");
  ...
  if (assignedTo) { conditions.push(`t.assigned_to = $${paramIdx++}`); params.push(assignedTo); }
  if (search) {
    conditions.push(`(t.subject ILIKE $${paramIdx} OR t.ticket_number ILIKE $${paramIdx} OR t.requester_name ILIKE $${paramIdx} OR t.requester_email ILIKE $${paramIdx})`);
    params.push(`%${search}%`);
    paramIdx++;
  }
  if (overdueOnly) { conditions.push("t.is_overdue = true"); }
...
ticketRoute.get("/:id", jwtAuth, tenantContext, requirePermission("tickets:read"), async (c) => {
  ...
  if (ticketResult.error || !ticketResult.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Ticket não encontrado" } }, 404);
  }
...
ticketRoute.post("/", jwtAuth, tenantContext, requirePermission("tickets:write"), async (c) => {
  const user = c.get("user");
  ...
  if (numberResult.error || !numberResult.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao gerar número do ticket" } }, 500);
  }
  ...
  if (data.category_id) {
    const slaResult = await query<{ sla_response_due: string; sla_resolution_due: string }>(
      "SELECT * FROM public.calculate_ticket_sla($1, $2)",
      [data.category_id, data.priority],
    );
  ...
  const result = await query<{ id: string }>(
    `INSERT INTO public.tickets (tenant_id, ticket_number, category_id, subject, description, status, priority, source,
     ...
     RETURNING id`,
    [
      user?.tenant_id ?? null, ticketNumber, data.category_id ?? null,
      data.subject, data.description, data.priority, data.source,
      data.requester_name, data.requester_email, data.requester_phone ?? null,
      data.assigned_to ?? null, JSON.stringify(data.tags), JSON.stringify(data.metadata),
      slaResponseDue, slaResolutionDue, user.sub,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar ticket" } }, 500);
  }
  ...
  await query(
    ...
    [user.sub, ticketId, JSON.stringify({ ticket_number: ticketNumber, subject: data.subject, priority: data.priority })],
  );

  return c.json({ id: ticketId, ticket_number: ticketNumber }, 201);
});

ticketRoute.put("/:id", jwtAuth, tenantContext, requirePermission("tickets:write"), async (c) => {
  const ticketId = c.req.param("id");
  const user = c.get("user");
  ...
  const fieldMap: Record<string, string> = {
    ...
    assigned_name: "assigned_name", rating: "rating", rating_comment: "rating_comment",
  };
  ...
  if (data.status === "resolved") {
    ...
    updateFields.push(`resolution_time_mins = EXTRACT(EPOCH FROM (timezone('utc'::text, now()) - created_at)) / 60`);
  ...
  if (data.priority || data.category_id) {
    const currentResult = await query<{ priority: string; category_id: string | null }>(
    ...
    if (currentResult.data?.rows[0]) {
      ...
      if (newCategoryId) {
        const slaResult = await query<{ sla_response_due: string; sla_resolution_due: string }>(
          "SELECT * FROM public.calculate_ticket_sla($1, $2)",
          [newCategoryId, newPriority],
        );
        if (slaResult.data?.rows[0]) {
          updateFields.push(`sla_response_due = $${paramIdx++}`, `sla_resolution_due = $${paramIdx++}`);
          params.push(slaResult.data.rows[0].sla_response_due, slaResult.data.rows[0].sla_resolution_due);
        }
      }
    }
  }
...
ticketRoute.delete("/:id", jwtAuth, tenantContext, requirePermission("tickets:write"), async (c) => {
  const ticketId = c.req.param("id");
  const user = c.get("user");
...
ticketRoute.get("/:id/comments", jwtAuth, tenantContext, requirePermission("tickets:read"), async (c) => {
  const ticketId = c.req.param("id");
  const result = await query(
    "SELECT id, tenant_id, ticket_id, author_id, author_name, author_type, body, is_internal, created_at FROM public.ticket_comments WHERE ticket_id = $1 ORDER BY created_at ASC",
    [ticketId],
...
ticketRoute.post("/:id/comments", jwtAuth, tenantContext, requirePermission("tickets:write"), async (c) => {
  const ticketId = c.req.param("id");
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    ...
    [user?.tenant_id ?? null, ticketId, user.sub, user.sub, data.body, data.is_internal],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao adicionar comentário" } }, 500);
  }
...
ticketRoute.delete("/:id/comments/:commentId", jwtAuth, tenantContext, requirePermission("tickets:write"), async (c) => {
...
ticketRoute.get("/stats/overview", jwtAuth, tenantContext, requirePermission("tickets:read"), async (c) => {
  const user = c.get("user");
  ...
  return c.json({
    ...
    sla: slaResult.data?.rows[0] ?? { overdue: "0", open_tickets: "0", avg_response_mins: null, avg_resolution_mins: null, avg_rating: null },
