...
errorReportRoute.post("/report", async (c) => {
  ...
  if (userId) {
    await query(
      "SELECT public.write_audit_log($1, NULL, 'error.report', 'error_report', $2, $3, NULL, NULL)",
      [
        userId,
        reportId,
        JSON.stringify({
...
errorReportRoute.put(
  ...
  async (c) => {
    ...
    if (user?.sub) {
      await query(
        "SELECT public.write_audit_log($1, NULL, 'error.report.update', 'error_report', $2, $3, NULL, NULL)",
        [user.sub, id, JSON.stringify(data)],
      );
    }
