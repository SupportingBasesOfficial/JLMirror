...
webhookRoute.get("/", jwtAuth, tenantContext, requirePermission("webhooks:read"), async (c) => {
  ...
  const countResult = await query(`SELECT COUNT(*)::int as total FROM public.webhooks WHERE ${whereClause}`, params);
...
webhookRoute.post("/", jwtAuth, tenantContext, requirePermission("webhooks:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    ...
    [
      ...
      data.url, data.method, JSON.stringify(data.events), JSON.stringify(data.headers),
      secret, data.is_active, data.max_retries, data.retry_delay_seconds,
      data.timeout_seconds, data.expected_status_code, user.sub,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar webhook" } }, 500);
  }

  await query(
    "SELECT public.write_audit_log($1, NULL, 'webhook.create', 'webhook', $2, $3, NULL, NULL)",
    [user.sub, result.data.rows[0].id, JSON.stringify({ name: data.name, url: data.url, events: data.events })],
  );
...
webhookRoute.put("/:id", jwtAuth, tenantContext, requirePermission("webhooks:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    ...
    secret: "secret", is_active: "is_active", is_verified: "is_verified",
    max_retries: "max_retries", retry_delay_seconds: "retry_delay_seconds",
    timeout_seconds: "timeout_seconds", expected_status_code: "expected_status_code",
  };
...
webhookRoute.delete("/:id", jwtAuth, tenantContext, requirePermission("webhooks:write"), async (c) => {
  const webhookId = c.req.param("id");
  const user = c.get("user");
...
webhookRoute.post("/:id/test", jwtAuth, tenantContext, requirePermission("webhooks:write"), async (c) => {
  const webhookId = c.req.param("id");
  const user = c.get("user");
  ...
  if (whResult.error || !whResult.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Webhook não encontrado" } }, 404);
  }

  const wh = whResult.data.rows[0] as {
    id: string; url: string; method: string; headers: Record<string, string>;
    secret: string | null; timeout_seconds: number; expected_status_code: number;
  };
  ...
  try {
    ...
    const timeout = setTimeout(() => controller.abort(), wh.timeout_seconds * 1000);
  ...
  await query(
    ...
    [user?.tenant_id ?? null, webhookId, testPayload, success ? "success" : "failed",
     statusCode || null, responseBody.substring(0, 5000), responseTimeMs, errorMsg],
  );
  ...
  await query(
    `UPDATE public.webhooks SET
     ...
     WHERE id = $4`,
    [success ? "success" : "failed", success ? 1 : 0, success ? 0 : 1, webhookId],
  );
...
webhookRoute.post("/trigger", jwtAuth, tenantContext, requirePermission("webhooks:write"), async (c) => {
  const user = c.get("user");
  ...
  const eventResult = await query<{ id: string }>(
    ...
    [user?.tenant_id ?? null, data.event_name, data.source_type ?? null, data.source_id ?? null, payloadStr],
  );
  ...
  for (const wh of matchedWebhooks as Array<{
    id: string; url: string; method: string; headers: Record<string, string>;
    secret: string | null; timeout_seconds: number; expected_status_code: number;
    max_retries: number; retry_delay_seconds: number;
  }>) {
  ...
  return c.json({ event_id: eventResult.data?.rows[0]?.id, triggered_webhooks: triggered }, 201);
});

// ========== Deliveries ==========

webhookRoute.get("/:id/deliveries", jwtAuth, tenantContext, requirePermission("webhooks:read"), async (c) => {
  const webhookId = c.req.param("id");
  const user = c.get("user");
...
webhookRoute.post("/:id/deliveries/:deliveryId/retry", jwtAuth, tenantContext, requirePermission("webhooks:write"), async (c) => {
  const webhookId = c.req.param("id");
  ...
  if (deliveryResult.error || !deliveryResult.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Entrega não encontrada" } }, 404);
  }

  const delivery = deliveryResult.data.rows[0] as {
    id: string; event_name: string; payload: string; attempt_number: number;
  };
  ...
  try {
    ...
    const headers: Record<string, string> = { "Content-Type": "application/json", ...wh.headers };
    if (wh.secret) {
      headers["X-Webhook-Signature"] = signPayload(wh.secret, delivery.payload);
    }
  ...
  await query(
    `UPDATE public.webhook_deliveries SET
     ...
     WHERE id = $7`,
    [success ? "success" : "failed", newAttempt, statusCode || null,
     responseBody.substring(0, 5000), responseTimeMs, errorMsg, deliveryId],
  );
...
webhookRoute.get("/stats/overview", jwtAuth, tenantContext, requirePermission("webhooks:read"), async (c) => {
  const user = c.get("user");
  ...
  return c.json({
    overview: overviewResult.data?.rows[0] ?? { total_webhooks: "0", active_webhooks: "0", verified_webhooks: "0", total_deliveries: "0", successful_deliveries: "0", failed_deliveries: "0" },
...
webhookRoute.post("/verify/:id", jwtAuth, tenantContext, requirePermission("webhooks:write"), async (c) => {
  const webhookId = c.req.param("id");
  const user = c.get("user");
  ...
  if (!secret) {
    return c.json({ error: { code: "NO_SECRET", message: "Webhook não possui secret configurado" } }, 400);
  }
  ...
  const valid = providedSig.length === expectedBuf.length && timingSafeEqual(providedSig, expectedBuf);

  if (valid) {
    await query("UPDATE public.webhooks SET is_verified = true WHERE id = $1", [webhookId]);
  }
