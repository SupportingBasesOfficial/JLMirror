...
function execSshCommand(
  ...
  return new Promise((resolve) => {
    exec(
      `ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 ${host} "${command.replace(/"/g, '\\"')}"`,
...
firewallRoute.get("/rules", requirePermission("firewall:read"), async (c) => {
...
firewallRoute.post("/rules", requirePermission("firewall:write"), async (c) => {
  const user = c.get("user");
...
firewallRoute.post("/apply", requirePermission("firewall:write"), async (c) => {
  const user = c.get("user");
  const body = await c.req.json<ApplyFirewallInput>();
  const parsed = applyFirewallSchema.safeParse(body);
  if (!parsed.success) {
  ...
  for (const cmd of commands) {
    ...
    const result = isLocal
      ? await new Promise<{ stdout: string; stderr: string; exitCode: number }>(
          (resolve) => {
            exec(
              cmd,
...
firewallRoute.get("/changes", requirePermission("firewall:read"), async (c) => {
  const user = c.get("user");
  const host = c.req.query("host");
...
firewallRoute.get("/hosts", requirePermission("firewall:read"), async (c) => {
  const user = c.get("user");
