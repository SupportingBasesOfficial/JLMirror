...
capacityRoute.get("/metrics", jwtAuth, tenantContext, requirePermission("capacity:read"), async (c) => {
  ...
  const conditions: string[] = ["tenant_id = $1", `recorded_at > timezone('utc'::text, now()) - ($2 || ' hours')::INTERVAL`];
  const params: unknown[] = [user?.tenant_id ?? null, hours.toString()];
  let paramIdx = 3;

  if (resourceName) { conditions.push(`resource_name = $${paramIdx++}`); params.push(resourceName); }
  ...
  if (result.error) {
    return c.json({ error: { code: "QUERY_ERROR", message: "Erro ao buscar métricas" } }, 500);
  }
...
capacityRoute.post("/metrics", jwtAuth, tenantContext, requirePermission("capacity:write"), async (c) => {
  const user = c.get("user");
  ...
  const labels: Record<string, unknown> = { resource_type: data.resource_type };
  if (data.resource_id) labels.resource_id = data.resource_id;
  if (data.max_capacity !== undefined) labels.max_capacity = data.max_capacity;
  if (data.utilization_pct !== undefined) labels.utilization_pct = data.utilization_pct;
  ...
  const result = await query<{ id: string }>(
    `INSERT INTO public.capacity_metrics (tenant_id, resource_name, metric_type, value, unit, labels, recorded_at)
     ...
     RETURNING id`,
    [
      user?.tenant_id ?? null, data.resource_name, data.metric_name,
      data.metric_value, data.metric_unit, JSON.stringify(labels),
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao inserir métrica" } }, 500);
  }
...
capacityRoute.post("/metrics/batch", jwtAuth, tenantContext, requirePermission("capacity:write"), async (c) => {
  const user = c.get("user");
  const body = await c.req.json<IngestMetricInput[]>();
  ...
  for (const m of metrics) {
    const labels: Record<string, unknown> = { resource_type: m.resource_type };
    if (m.resource_id) labels.resource_id = m.resource_id;
    if (m.max_capacity !== undefined) labels.max_capacity = m.max_capacity;
    if (m.utilization_pct !== undefined) labels.utilization_pct = m.utilization_pct;
    if (m.metadata) Object.assign(labels, m.metadata);
    const result = await query(
      `INSERT INTO public.capacity_metrics (tenant_id, resource_name, metric_type, value, unit, labels, recorded_at)
       VALUES ($1, $2, $3, $4, $5, $6, timezone('utc'::text, now()))`,
      [
        user?.tenant_id ?? null, m.resource_name, m.metric_name,
        m.metric_value, m.metric_unit, JSON.stringify(labels),
      ],
    );
...
capacityRoute.get("/thresholds", jwtAuth, tenantContext, requirePermission("capacity:read"), async (c) => {
  const user = c.get("user");
  const result = await query(
...
capacityRoute.post("/thresholds", jwtAuth, tenantContext, requirePermission("capacity:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    `INSERT INTO public.capacity_thresholds (tenant_id, resource_type, resource_name, warning_pct, critical_pct, is_active)
     ...
     RETURNING id`,
    [user?.tenant_id ?? null, data.resource_type, data.resource_name, data.warning_pct, data.critical_pct, data.is_active],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar threshold" } }, 500);
  }

  await query(
    "SELECT public.write_audit_log($1, NULL, 'capacity.threshold.create', 'capacity_threshold', $2, $3, NULL, NULL)",
    [user.sub, result.data.rows[0].id, JSON.stringify({ resource: data.resource_name, type: data.resource_type })],
  );

  return c.json({ id: result.data.rows[0].id }, 201);
});

capacityRoute.put("/thresholds/:id", jwtAuth, tenantContext, requirePermission("capacity:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    warning_pct: "warning_pct", critical_pct: "critical_pct", is_active: "is_active",
  };
...
capacityRoute.delete("/thresholds/:id", jwtAuth, tenantContext, requirePermission("capacity:write"), async (c) => {
  const thresholdId = c.req.param("id");
  const user = c.get("user");
...
capacityRoute.get("/forecast", jwtAuth, tenantContext, requirePermission("capacity:read"), async (c) => {
  const user = c.get("user");
  ...
  if (resourceType && resourceName) {
    const metricType = c.req.query("metric_type") ?? c.req.query("metric_name") ?? "utilization_pct";
    const result = await query(
    ...
    if (result.error || !result.data?.rows[0]) {
      return c.json({ error: { code: "FORECAST_ERROR", message: "Erro ao calcular previsão" } }, 500);
    }
    ...
    await query(
      ...
      [
        user?.tenant_id ?? null, resourceType, resourceName, metricType,
        row.current_value, row.predicted_7d, row.predicted_30d, row.predicted_90d,
        row.slope, row.r_squared, row.days_until_capacity, row.confidence,
      ],
    );
...
capacityRoute.get("/reports", jwtAuth, tenantContext, requirePermission("capacity:read"), async (c) => {
  const user = c.get("user");
  const result = await query(
...
capacityRoute.post("/reports", jwtAuth, tenantContext, requirePermission("capacity:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    `INSERT INTO public.capacity_reports (tenant_id, name, report_type, date_range_start, date_range_end, status, is_scheduled, cron_expression, generated_by)
     ...
     RETURNING id`,
    [
      user?.tenant_id ?? null, data.name, data.report_type,
      data.date_range_start ?? null, data.date_range_end ?? null,
      data.is_scheduled, data.cron_expression ?? null, user.sub,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar relatório" } }, 500);
  }

  const reportId = result.data.rows[0].id;

  // MVP: Gera relatório síncrono — Enterprise: job assíncrono
  const startDate = data.date_range_start ? new Date(data.date_range_start) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
  const endDate = data.date_range_end ? new Date(data.date_range_end) : new Date();
  ...
  await query(
    ...
    [user.sub, reportId, JSON.stringify({ name: data.name, type: data.report_type })],
  );

  return c.json({ id: reportId, status: "completed", summary }, 201);
});

capacityRoute.delete("/reports/:id", jwtAuth, tenantContext, requirePermission("capacity:write"), async (c) => {
...
capacityRoute.get("/stats", jwtAuth, tenantContext, requirePermission("capacity:read"), async (c) => {
  const user = c.get("user");
  ...
  return c.json({
    overview: overviewResult.data?.rows[0] ?? { total_resources: "0", total_types: "0", critical_count: "0", warning_count: "0" },
    by_type: byTypeResult.data?.rows ?? [],
    top_utilized: topUtilized.data?.rows ?? [],
    forecasts: forecastsResult.data?.rows[0] ?? { total: "0", within_30d: "0", within_90d: "0", high_confidence: "0" },
  });
});
