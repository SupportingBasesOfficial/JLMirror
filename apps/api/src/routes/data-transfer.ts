...
dataTransferRoute.get("/whitelist", jwtAuth, tenantContext, requirePermission("data_transfer:read"), async (c) => {
...
dataTransferRoute.get("/templates", jwtAuth, tenantContext, requirePermission("data_transfer:read"), async (c) => {
...
dataTransferRoute.post("/templates", jwtAuth, tenantContext, requirePermission("data_transfer:write"), async (c) => {
  const user = c.get("user");
  ...
  if (!wlResult.data?.rows[0]) {
    return c.json({ error: { code: "FORBIDDEN_TABLE", message: `Tabela ${data.source_table} não permite exportação` } }, 403);
  }
  ...
  const result = await query<{ id: string }>(
    ...
    [
      ...
      data.source_table, data.format, JSON.stringify(data.columns), JSON.stringify(data.filters),
      data.include_headers, data.delimiter, data.encoding, data.is_active, user.sub,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar template" } }, 500);
  }

  return c.json({ id: result.data.rows[0].id }, 201);
});

dataTransferRoute.put("/templates/:id", jwtAuth, tenantContext, requirePermission("data_transfer:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    name: "name", description: "description", source_table: "source_table",
    format: "format", include_headers: "include_headers", delimiter: "delimiter",
    encoding: "encoding", is_active: "is_active",
  };
...
dataTransferRoute.delete("/templates/:id", jwtAuth, tenantContext, requirePermission("data_transfer:write"), async (c) => {
  const templateId = c.req.param("id");
  const user = c.get("user");
...
dataTransferRoute.get("/exports", jwtAuth, tenantContext, requirePermission("data_transfer:read"), async (c) => {
  const user = c.get("user");
...
dataTransferRoute.post("/exports", jwtAuth, tenantContext, requirePermission("data_transfer:write"), async (c) => {
  const user = c.get("user");
  ...
  const maxRows = (wlResult.data.rows[0] as { max_export_rows: number }).max_export_rows;
  ...
  const exportResult = await query<{ id: string }>(
    ...
    [
      user?.tenant_id ?? null, data.template_id ?? null, data.name,
      data.source_table, data.format, JSON.stringify(data.columns), JSON.stringify(data.filters),
      user.sub,
    ],
  );
  ...
  try {
      ...
      if (rows.length > 0) {
        ...
        for (const row of rows) {
          const values = headers.map((h) => {
            const val = (row as Record<string, unknown>)[h as keyof typeof row];
    ...
    await query(
      ...
      [user.sub, exportId, JSON.stringify({ table: data.source_table, format: data.format, rows: rows.length })],
    );
    ...
    return c.json({ error: { code: "EXPORT_ERROR", message: errorMsg } }, 500);
  }
});

dataTransferRoute.get("/exports/:id", jwtAuth, tenantContext, requirePermission("data_transfer:read"), async (c) => {
  ...
  if (!result.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Export não encontrado" } }, 404);
  }

  return c.json({ export: result.data.rows[0] });
});

dataTransferRoute.delete("/exports/:id", jwtAuth, tenantContext, requirePermission("data_transfer:write"), async (c) => {
  const exportId = c.req.param("id");
  const user = c.get("user");
...
dataTransferRoute.get("/imports", jwtAuth, tenantContext, requirePermission("data_transfer:read"), async (c) => {
  const user = c.get("user");
  const status = c.req.query("status");
...
dataTransferRoute.post("/imports", jwtAuth, tenantContext, requirePermission("data_transfer:write"), async (c) => {
  const user = c.get("user");
  ...
  if (!wlResult.data?.rows[0]) {
    return c.json({ error: { code: "FORBIDDEN_TABLE", message: `Tabela ${data.target_table} não permite importação` } }, 403);
  }
  ...
  const result = await query<{ id: string }>(
    ...
    [
      user?.tenant_id ?? null, data.name, data.target_table, data.format,
      data.file_path ?? null, JSON.stringify(data.column_mapping), JSON.stringify(data.options),
      user.sub,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar import" } }, 500);
  }

  return c.json({ id: result.data.rows[0].id }, 201);
});

dataTransferRoute.post("/imports/:id/run", jwtAuth, tenantContext, requirePermission("data_transfer:write"), async (c) => {
  ...
  if (!importResult.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Import não encontrado" } }, 404);
  }

  const imp = importResult.data.rows[0] as {
    id: string; name: string; target_table: string; format: string;
    column_mapping: Record<string, string>; options: Record<string, unknown>;
  };
      ...
      try {
        ...
        for (const [sourceCol, targetCol] of Object.entries(mapping)) {
          if (sourceCol in record) {
            mappedRecord[targetCol as keyof typeof mappedRecord] = record[sourceCol as keyof typeof record];
          }
        }

        // Se não há mapping, usa as chaves do registro diretamente
        const finalRecord = Object.keys(mappedRecord).length > 0 ? mappedRecord : record;
        ...
        failed++;
        errors.push(`Linha ${i + 1}: ${err instanceof Error ? err.message : "erro"}`);
      }
    }
  }
  ...
  await query(
    `UPDATE public.data_imports SET
     ...
     WHERE id = $9`,
    [status, rows.length, successful, failed, skipped,
  ...
  await query(
    ...
    [user.sub, importId, JSON.stringify({ table: imp.target_table, total: rows.length, success: successful, failed })],
  );
...
dataTransferRoute.delete("/imports/:id", jwtAuth, tenantContext, requirePermission("data_transfer:write"), async (c) => {
  const importId = c.req.param("id");
  const user = c.get("user");
...
dataTransferRoute.get("/stats/overview", jwtAuth, tenantContext, requirePermission("data_transfer:read"), async (c) => {
  const user = c.get("user");
  ...
  return c.json({
    exports: exportStats.data?.rows[0] ?? { total_exports: "0", completed: "0", failed: "0", processing: "0", total_rows_exported: "0", total_size_bytes: "0" },
    imports: importStats.data?.rows[0] ?? { total_imports: "0", completed: "0", failed: "0", partial: "0", processing: "0", total_rows_imported: "0", successful_rows: "0", failed_rows: "0" },
