...
dataTransferRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro no overview data-transfer", {
        tenantId,
...
dataTransferRoute.get(
  ...
  async (c) => {
    try {
      ...
      logger.error("Erro ao listar whitelist", {
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
        { error: { code: "INTERNAL_ERROR", message: "Erro interno" } },
  ...
  async (c) => {
    ...
    try {
      ...
      logger.info("Template criado", {
        templateId: result.data.rows[0].id,
        name: data.name,
...
dataTransferRoute.put(
  ...
  async (c) => {
    ...
    try {
      ...
      const result = await query(
        `UPDATE public.data_export_templates SET ${updateFields.join(", ")} WHERE id = $${paramIdx++} AND tenant_id = $${paramIdx++} RETURNING id`,
        params,
      );
...
dataTransferRoute.delete(
  ...
  async (c) => {
    ...
    try {
      const result = await query(
        "DELETE FROM public.data_export_templates WHERE id = $1 AND tenant_id = $2 RETURNING id",
      ...
      return c.json(
        {
          error: { code: "DELETE_ERROR", message: "Erro ao excluir template" },
        },
        500,
      );
    }
...
dataTransferRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro ao listar exports", {
        tenantId: user?.tenant_id,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
...
dataTransferRoute.post(
  ...
  async (c) => {
    ...
    try {
      ...
      if (!exportId) {
        return c.json(
          { error: { code: "CREATE_ERROR", message: "Erro ao criar export" } },
          500,
        );
      }
      ...
      return c.json(
        {
          error: {
            code: "INTERNAL_ERROR",
            message: "Erro ao processar export",
          },
        },
        500,
      );
...
dataTransferRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro ao buscar export", {
        exportId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
...
dataTransferRoute.delete(
  ...
  async (c) => {
    ...
    try {
      const result = await query(
        "DELETE FROM public.data_exports WHERE id = $1 AND tenant_id = $2 RETURNING id",
        [exportId, user?.tenant_id ?? null],
      );
      if (result.error || !result.data?.rows[0]) {
        return c.json(
      ...
      logger.error("Erro ao deletar export", {
        exportId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
        { error: { code: "DELETE_ERROR", message: "Erro ao excluir export" } },
        500,
      );
    }
  },
...
dataTransferRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro ao listar imports", {
        tenantId: user?.tenant_id,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
    ...
    try {
      ...
      const result = await query<{ id: string }>(
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending', $8) RETURNING id`,
        [
          user?.tenant_id ?? null,
          data.name ?? `Import ${data.target_table}`,
          data.target_table,
          data.format,
          data.file_path ?? null,
      ...
      logger.info("Import criado", {
        importId: result.data.rows[0].id,
...
dataTransferRoute.post(
  ...
  async (c) => {
    ...
    try {
      ...
      // Recebe dados do body com validação
      const parsedBody = await safeJsonBody(c);
      if (!parsedBody.success) return parsedBody.response;
      const parsed = runImportSchema.safeParse(parsedBody.data);
      if (!parsed.success) {
        return c.json(
          {
            error: {
      ...
      logger.error("Erro ao executar import", {
        importId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
        { error: { code: "IMPORT_ERROR", message: "Erro ao executar import" } },
        500,
      );
    }
  },
...
dataTransferRoute.delete(
  ...
  async (c) => {
    ...
    try {
      const result = await query(
        "DELETE FROM public.data_imports WHERE id = $1 AND tenant_id = $2 RETURNING id",
        [importId, user?.tenant_id ?? null],
      );
      if (result.error || !result.data?.rows[0]) {
        return c.json(
      ...
      logger.error("Erro ao deletar import", {
        importId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
        { error: { code: "DELETE_ERROR", message: "Erro ao excluir import" } },
        500,
      );
    }
  },
...
dataTransferRoute.get(
  ...
  async (c) => {
    ...
    try {
      // Paraleliza 5 queries (antes seriais)
      const [
        exportStats,
        importStats,
        recentExports,
      ...
      logger.error("Erro no stats data-transfer", {
        tenantId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
