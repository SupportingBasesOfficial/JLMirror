...
executionsRoute.get("/", jwtAuth, tenantContext, requirePermission("scripts:read"), async (c) => {
  ...
  const countResult = await query(`SELECT COUNT(*)::int as total FROM public.script_executions WHERE ${whereClause}`, params);
...
executionsRoute.get("/pending", jwtAuth, tenantContext, requirePermission("scripts:approve"), async (c) => {
  const user = c.get("user");
...
executionsRoute.get("/:id", jwtAuth, tenantContext, requirePermission("scripts:read"), async (c) => {
...
executionsRoute.post("/:id/approve", jwtAuth, tenantContext, requirePermission("scripts:approve"), async (c) => {
  const executionId = c.req.param("id");
  const user = c.get("user");
  ...
  const result = await query<{ id: string; status: string }>(
    ...
    [executionId, user.sub, parsed.data.decision, parsed.data.comment ?? null],
  );

  if (result.error) {
    return c.json({ error: { code: "APPROVAL_ERROR", message: result.error.message } }, 400);
  }

  // Auditoria
  await query(
    "SELECT public.write_audit_log($1, NULL, 'script.approve', 'execution', $2, $3, NULL, NULL)",
    [user.sub, executionId, JSON.stringify({ decision: parsed.data.decision, comment: parsed.data.comment })],
  );
...
executionsRoute.post("/:id/run", jwtAuth, tenantContext, requirePermission("scripts:execute"), async (c) => {
  const executionId = c.req.param("id");
  const user = c.get("user");
  ...
  if (execution.status !== "approved") {
    return c.json({ error: { code: "INVALID_STATUS", message: `Execução precisa estar aprovada (atual: ${execution.status})` } }, 400);
  }
  ...
  try {
    ...
    const result = await new Promise<{ code: number; stdout: string; stderr: string }>((resolve) => {
      const proc = exec(cmd, { timeout: script.timeout_seconds * 1000 }, (err, out, errOut) => {
  ...
  await query(
    `UPDATE public.script_executions SET
       ...
       stdout = $4, stderr = $5
     WHERE id = $6`,
    [status, exitCode, durationMs, stdout.substring(0, 50000), stderr.substring(0, 50000), executionId],
  );

  await query(
    "SELECT public.write_audit_log($1, NULL, 'script.run', 'execution', $2, $3, NULL, NULL)",
    [user.sub, executionId, JSON.stringify({ status, exit_code: exitCode, duration_ms: durationMs })],
  );
...
executionsRoute.post("/:id/cancel", jwtAuth, tenantContext, requirePermission("scripts:execute"), async (c) => {
  const executionId = c.req.param("id");
  const user = c.get("user");
  ...
  if (status !== "pending" && status !== "approved") {
    return c.json({ error: { code: "INVALID_STATUS", message: `Não é possível cancelar execução com status: ${status}` } }, 400);
  }
