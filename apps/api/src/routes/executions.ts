...
executionsRoute.get("/", requirePermission("scripts:read"), async (c) => {
  ...
  try {
    ...
    logger.error("Erro ao listar execucoes", {
      tenantId,
...
executionsRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro ao listar execucoes pendentes", {
        tenantId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
...
executionsRoute.get("/:id", requirePermission("scripts:read"), async (c) => {
  ...
  try {
    ...
    logger.error("Erro ao buscar execucao", {
      executionId,
      tenantId,
      error: error instanceof Error ? error.message : String(error),
    });
...
executionsRoute.post(
  ...
  async (c) => {
    ...
    try {
      ...
      const parsed = approveExecutionSchema.safeParse(bodyResult.data);
      if (!parsed.success) {
        return c.json(
      ...
      logger.error("Erro ao aprovar execucao", {
        executionId,
        tenantId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
        { error: { code: "INTERNAL_ERROR", message: "Erro ao aprovar" } },
        500,
      );
    }
  },
...
executionsRoute.post(
  ...
  async (c) => {
    ...
    try {
      ...
      return c.json(
        { error: { code: "INTERNAL_ERROR", message: "Erro ao executar" } },
        500,
      );
    }
  },
...
executionsRoute.post(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro ao cancelar execucao", {
        executionId,
        tenantId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
        { error: { code: "INTERNAL_ERROR", message: "Erro ao cancelar" } },
        500,
      );
    }
  },
