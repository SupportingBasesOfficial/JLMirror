...
// GET /api/v1/feature-flags/overview — overview do modulo
...
featureFlagRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro ao listar feature flags", {
...
featureFlagRoute.post(
  ...
  async (c) => {
    ...
    const parsed = createFeatureFlagSchema.safeParse(parsedBody.data);
    if (!parsed.success) {
      return c.json(
        {
          error: {
    ...
    const data = parsed.data as CreateFeatureFlagInput;
    ...
    try {
      ...
      const flagId = result.data.rows[0].id;
      ...
      logger.info("Feature flag criada", { flagId, key: data.key, tenantId });

      return c.json({ id: flagId }, 201);
    } catch (error) {
      logger.error("Erro ao criar feature flag", {
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
...
featureFlagRoute.put(
  ...
  async (c) => {
    ...
    const parsedBody = await safeJsonBody(c);
    if (!parsedBody.success) return parsedBody.response;
    const parsed = updateFeatureFlagSchema.safeParse(parsedBody.data);
    if (!parsed.success) {
      return c.json(
        {
          error: {
    ...
    const data = parsed.data as UpdateFeatureFlagInput;
    ...
    try {
      ...
      if (result.data?.rowCount === 0) {
        return c.json(
          { error: { code: "NOT_FOUND", message: "Flag não encontrada" } },
          404,
        );
      }

      logger.info("Feature flag atualizada", { flagId, tenantId });
...
featureFlagRoute.delete(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.info("Feature flag removida", { flagId, tenantId });

      return c.json({ deleted: true });
    } catch (error) {
      logger.error("Erro ao remover feature flag", {
        flagId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
...
featureFlagRoute.post("/evaluate", rateLimitWrite, async (c) => {
  const user = c.get("user");
  const tenantId = user?.tenant_id ?? null;
  const fallbackUserId = user?.sub ?? "anonymous";

  const parsedBody = await safeJsonBody(c);
  if (!parsedBody.success) return parsedBody.response;
  const parsed = evaluateFlagSchema.safeParse(parsedBody.data);
  if (!parsed.success) {
    return c.json(
      {
        error: {
  ...
  const data = parsed.data as EvaluateFlagInput;
  const userId = data.user_id ?? fallbackUserId;
  ...
  try {
    ...
    if (overrideResult.data?.rows[0]) {
      ...
      // Atualiza contadores + registra evento em paralelo
      await Promise.all([
        query(
    ...
    logger.error("Erro ao avaliar feature flag", {
      key: data.key,
      error: error instanceof Error ? error.message : String(error),
    });
    return c.json(
...
featureFlagRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro ao listar overrides", {
        flagId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
...
featureFlagRoute.post(
  ...
  async (c) => {
    ...
    const parsedBody = await safeJsonBody(c);
    if (!parsedBody.success) return parsedBody.response;
    const parsed = createOverrideSchema.safeParse(parsedBody.data);
    if (!parsed.success) {
      return c.json(
        {
          error: {
    ...
    const data = parsed.data as CreateOverrideInput;
    ...
    try {
      ...
      if (result.error || !result.data?.rows[0]) {
        return c.json(
          {
            error: { code: "CREATE_ERROR", message: "Erro ao criar override" },
          },
          500,
        );
      }

      logger.info("Override criado", {
        overrideId: result.data.rows[0].id,
        flagId: data.flag_id,
        tenantId,
      });
      ...
      logger.error("Erro ao criar override", {
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
        { error: { code: "CREATE_ERROR", message: "Erro ao criar override" } },
...
featureFlagRoute.delete(
  ...
  async (c) => {
    ...
    try {
      ...
      if (result.data?.rowCount === 0) {
        return c.json(
          { error: { code: "NOT_FOUND", message: "Override não encontrado" } },
          404,
        );
      }

      logger.info("Override removido", { overrideId, tenantId });

      return c.json({ deleted: true });
    } catch (error) {
      logger.error("Erro ao remover override", {
        overrideId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
...
featureFlagRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Erro ao listar events", {
        flagId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
...
featureFlagRoute.get(
  ...
  async (c) => {
    ...
    try {
      ...
      const [overviewResult, topFlags, recentEvents, overrideCount] =
        await Promise.all([
          query(
            `SELECT
      ...
      logger.error("Erro ao buscar stats", {
        tenantId,
        error: error instanceof Error ? error.message : String(error),
      });
      return c.json(
