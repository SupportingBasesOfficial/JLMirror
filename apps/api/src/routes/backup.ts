...
backupRoute.get("/jobs", requirePermission("backup:read"), async (c) => {
  const user = c.get("user");
...
backupRoute.post("/jobs", requirePermission("backup:write"), async (c) => {
  const user = c.get("user");
...
backupRoute.put("/jobs/:id", requirePermission("backup:write"), async (c) => {
  const jobId = c.req.param("id");
  const user = c.get("user");
    ...
    if (tarResult.exitCode !== 0) {
      ...
      await query(
        ...
        [
          user.sub,
          jobId,
          JSON.stringify({ snapshot_id: snapshotId, error: tarResult.stderr }),
        ],
      );
      return c.json(
        {
          error: {
            code: "BACKUP_FAILED",
            message: `Erro no backup: ${tarResult.stderr}`,
          },
        },
        500,
      );
...
backupRoute.get("/snapshots", requirePermission("backup:read"), async (c) => {
  const user = c.get("user");
...
backupRoute.post("/restore", requirePermission("backup:write"), async (c) => {
  const user = c.get("user");
...
backupRoute.get("/restores", requirePermission("backup:read"), async (c) => {
  const user = c.get("user");
  const limit = Math.min(parseInt(c.req.query("limit") ?? "50", 10), 200);
...
backupRoute.get("/stats", requirePermission("backup:read"), async (c) => {
  const user = c.get("user");
