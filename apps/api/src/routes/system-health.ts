...
systemHealthRoute.get("/checks", jwtAuth, tenantContext, requirePermission("health:read"), async (c) => {
  ...
  if (serviceType) { conditions.push(`service_type = $${paramIdx++}`); params.push(serviceType); }
  if (status) { conditions.push(`last_status = $${paramIdx++}`); params.push(status); }
...
systemHealthRoute.post("/checks", jwtAuth, tenantContext, requirePermission("health:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    ...
    [user?.tenant_id ?? null, data.name, data.service_type, data.endpoint ?? null,
     data.check_interval_seconds, data.timeout_seconds, data.expected_status_code ?? null,
     data.is_active, JSON.stringify(data.metadata)],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar health check" } }, 500);
  }

  return c.json({ id: result.data.rows[0].id }, 201);
});

systemHealthRoute.put("/checks/:id", jwtAuth, tenantContext, requirePermission("health:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    name: "name", service_type: "service_type", endpoint: "endpoint",
    check_interval_seconds: "check_interval_seconds", timeout_seconds: "timeout_seconds",
    expected_status_code: "expected_status_code", is_active: "is_active",
  };
...
systemHealthRoute.delete("/checks/:id", jwtAuth, tenantContext, requirePermission("health:write"), async (c) => {
  const checkId = c.req.param("id");
  const user = c.get("user");
...
systemHealthRoute.post("/checks/:id/run", jwtAuth, tenantContext, requirePermission("health:write"), async (c) => {
  const checkId = c.req.param("id");
  const user = c.get("user");
  ...
  if (checkResult.error || !checkResult.data?.rows[0]) {
    return c.json({ error: { code: "NOT_FOUND", message: "Health check não encontrado" } }, 404);
  }

  const check = checkResult.data.rows[0] as {
    id: string; name: string; service_type: string; endpoint: string | null;
    timeout_seconds: number; expected_status_code: number | null;
  ...
  try {
    if (check.service_type === "api" || check.service_type === "external_api" || check.service_type === "webhook") {
      if (!check.endpoint) {
        status = "degraded";
        errorMsg = "Endpoint não configurado";
      } else {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), check.timeout_seconds * 1000);
        const res = await fetch(check.endpoint, { signal: controller.signal, method: "GET" });
        clearTimeout(timeout);
        responseTimeMs = Date.now() - startTime;

        if (check.expected_status_code && res.status !== check.expected_status_code) {
          status = "degraded";
  ...
  const newConsecutiveFailures = status === "healthy" ? 0 : check.consecutive_failures + 1;
  const newConsecutiveSuccesses = status === "healthy" ? check.consecutive_successes + 1 : 0;
  ...
  await query(
    `UPDATE public.system_health_checks
     ...
     WHERE id = $6`,
    [status, responseTimeMs, errorMsg, newConsecutiveFailures, newConsecutiveSuccesses, checkId],
  );
  ...
  if (newConsecutiveFailures >= 3) {
    ...
    if (!existingIncident.data?.rows[0]) {
      const incNumberResult = await query<{ generate_incident_number: string }>(
      ...
      if (incNumberResult.data?.rows[0]) {
        await query(
          ...
          [
            user?.tenant_id ?? null, incNumberResult.data.rows[0].generate_incident_number,
            checkId, `Serviço down: ${check.name}`,
            ...
            JSON.stringify([check.name]), user.sub,
          ],
        );
      }
    }
...
systemHealthRoute.get("/incidents", jwtAuth, tenantContext, requirePermission("health:read"), async (c) => {
  const user = c.get("user");
  const status = c.req.query("status");
...
systemHealthRoute.post("/incidents", jwtAuth, tenantContext, requirePermission("health:write"), async (c) => {
  const user = c.get("user");
  ...
  if (numberResult.error || !numberResult.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao gerar número" } }, 500);
  }
  ...
  const result = await query<{ id: string }>(
    ...
    [
      user?.tenant_id ?? null, incidentNumber, data.health_check_id ?? null,
      data.title, data.description ?? null, data.severity, data.status,
      JSON.stringify(data.affected_services), data.impact ?? null, data.is_scheduled,
      data.scheduled_start ?? null, data.scheduled_end ?? null, user.sub,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar incidente" } }, 500);
  }

  await query(
    "SELECT public.write_audit_log($1, NULL, 'health.incident.create', 'system_incident', $2, $3, NULL, NULL)",
    [user.sub, result.data.rows[0].id, JSON.stringify({ incident_number: incidentNumber, title: data.title, severity: data.severity })],
  );

  return c.json({ id: result.data.rows[0].id, incident_number: incidentNumber }, 201);
});

systemHealthRoute.put("/incidents/:id", jwtAuth, tenantContext, requirePermission("health:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    ...
    status: "status", root_cause: "root_cause", resolution_notes: "resolution_notes", impact: "impact",
  };
  ...
  if (data.status === "identified") {
    updateFields.push(`identified_at = COALESCE(identified_at, timezone('utc'::text, now()))`);
  } else if (data.status === "resolved") {
    updateFields.push(`resolved_at = timezone('utc'::text, now())`);
    updateFields.push(`duration_mins = EXTRACT(EPOCH FROM (timezone('utc'::text, now()) - started_at)) / 60`);
  }
...
systemHealthRoute.get("/metrics", jwtAuth, tenantContext, requirePermission("health:read"), async (c) => {
  const user = c.get("user");
  ...
  if (metricType) { conditions.push(`metric_type = $${paramIdx++}`); params.push(metricType); }
...
systemHealthRoute.post("/metrics", jwtAuth, tenantContext, requirePermission("health:write"), async (c) => {
  const user = c.get("user");
  ...
  if (data.threshold_critical !== undefined && data.value >= data.threshold_critical) {
    status = "critical";
  } else if (data.threshold_warning !== undefined && data.value >= data.threshold_warning) {
    status = "warning";
  }
  ...
  const result = await query<{ id: string }>(
    `INSERT INTO public.system_metric_snapshots (tenant_id, metric_name, metric_type, value, unit, labels, threshold_warning, threshold_critical, status)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id`,
    [
      user?.tenant_id ?? null, data.metric_name, data.metric_type, data.value,
      data.unit, JSON.stringify(data.labels), data.threshold_warning ?? null,
      data.threshold_critical ?? null, status,
    ],
  );

  if (result.error || !result.data?.rows[0]) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao registrar métrica" } }, 500);
  }
...
systemHealthRoute.get("/diagnostics", jwtAuth, tenantContext, requirePermission("health:read"), async (c) => {
  const user = c.get("user");

  const dbCheckResult = await query("SELECT 1 as ok, now() as server_time, version() as pg_version");
  ...
  return c.json({
    ...
    checks: checksSummary.data?.rows[0] ?? { total_checks: "0", healthy: "0", degraded: "0", down: "0", unknown: "0", avg_response_ms: null },
    ...
    runtime: {
      ...
      memory_total_mb: Math.floor(process.memoryUsage().heapTotal / 1024 / 1024),
    },
  });
});
...
systemHealthRoute.get("/score", jwtAuth, tenantContext, requirePermission("health:read"), httpCache(60), async (c) => {
  const user = c.get("user");
...
systemHealthRoute.get("/stats", jwtAuth, tenantContext, requirePermission("health:read"), async (c) => {
  const user = c.get("user");
  ...
  return c.json({
    overview: overviewResult.data?.rows[0] ?? { total_checks: "0", healthy: "0", degraded: "0", down: "0", avg_response_ms: null },
    incidents: incidentsResult.data?.rows[0] ?? { active_incidents: "0", resolved_incidents: "0", critical_active: "0", major_active: "0", avg_duration_mins: null, total_incidents: "0" },
