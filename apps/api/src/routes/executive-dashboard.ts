...
executiveDashboardRoute.get("/overview", httpCache(60), async (c) => {
  ...
  const totalDevices = safeCount(await query("SELECT COUNT(*) as count FROM public.devices WHERE tenant_id = $1", [tenantId]));
  const onlineDevices = safeCount(await query("SELECT COUNT(*) as count FROM public.devices WHERE tenant_id = $1 AND status = 'active'", [tenantId]));
  const offlineDevices = safeCount(await query("SELECT COUNT(*) as count FROM public.devices WHERE tenant_id = $1 AND status = 'inactive'", [tenantId]));
  const warningDevices = safeCount(await query("SELECT COUNT(*) as count FROM public.devices WHERE tenant_id = $1 AND status = 'warning'", [tenantId]));
  const uptimePct = totalDevices > 0 ? Math.round((onlineDevices / totalDevices) * 100 * 100) / 100 : 100;

  // Tickets — tabela é public.tickets (não support_tickets)
  const totalTickets = safeCount(await query("SELECT COUNT(*) as count FROM public.tickets WHERE tenant_id = $1", [tenantId]));
  const openTickets = safeCount(await query("SELECT COUNT(*) as count FROM public.tickets WHERE tenant_id = $1 AND status IN ('open','in_progress','waiting_customer')", [tenantId]));
  const resolvedTickets = safeCount(await query("SELECT COUNT(*) as count FROM public.tickets WHERE tenant_id = $1 AND status = 'resolved'", [tenantId]));
  const criticalTickets = safeCount(await query("SELECT COUNT(*) as count FROM public.tickets WHERE tenant_id = $1 AND priority = 'urgent' AND status NOT IN ('resolved','closed','cancelled')", [tenantId]));
  const ticketResolutionRate = totalTickets > 0 ? Math.round((resolvedTickets / totalTickets) * 100) : 0;

  // Assets
  const totalAssets = safeCount(await query("SELECT COUNT(*) as count FROM public.assets WHERE tenant_id = $1", [tenantId]));
  const activeAssets = safeCount(await query("SELECT COUNT(*) as count FROM public.assets WHERE tenant_id = $1 AND status = 'active'", [tenantId]));
  ...
  const complianceRow = complianceScansResult.data?.rows?.[0] ?? { total: "0", passed: "0", failed: "0" };
  ...
  // Capacity
  const capacityAlerts = safeCount(await query("SELECT COUNT(*) as count FROM public.capacity_forecasts WHERE tenant_id = $1 AND confidence IN ('high','medium')", [tenantId]));

  // SSL — usa view ssl_certificates_with_status para status calculado
  const sslExpiring = safeCount(await query("SELECT COUNT(*) as count FROM public.ssl_certificates_with_status WHERE tenant_id = $1 AND status = 'expiring_soon'", [tenantId]));
  const sslExpired = safeCount(await query("SELECT COUNT(*) as count FROM public.ssl_certificates_with_status WHERE tenant_id = $1 AND status = 'expired'", [tenantId]));

  // Firewall
  const firewallRules = safeCount(await query("SELECT COUNT(*) as count FROM public.firewall_rules WHERE tenant_id = $1 AND is_active = true", [tenantId]));

  // Backups — tabela é public.backup_snapshots (não backups)
  const totalBackups = safeCount(await query("SELECT COUNT(*) as count FROM public.backup_snapshots WHERE tenant_id = $1", [tenantId]));
  const successfulBackups = safeCount(await query("SELECT COUNT(*) as count FROM public.backup_snapshots WHERE tenant_id = $1 AND status IN ('completed','verified')", [tenantId]));

  // KB
  const kbArticles = safeCount(await query("SELECT COUNT(*) as count FROM public.kb_articles WHERE tenant_id = $1 AND status = 'published'", [tenantId]));

  // Scheduled Tasks
  const activeTasks = safeCount(await query("SELECT COUNT(*) as count FROM public.scheduled_tasks WHERE tenant_id = $1 AND is_active = true", [tenantId]));

  // API Keys
  const activeApiKeys = safeCount(await query("SELECT COUNT(*) as count FROM public.api_keys WHERE tenant_id = $1 AND is_active = true", [tenantId]));

  // Webhooks
  const activeWebhooks = safeCount(await query("SELECT COUNT(*) as count FROM public.webhooks WHERE tenant_id = $1 AND is_active = true", [tenantId]));

  // Notifications — tabela é public.notification_log, sem is_read; conta enviadas nas últimas 24h
  const unreadNotifications = safeCount(await query(
  ...
  // System Health
  const healthChecks = safeRows(await query("SELECT COUNT(*) as count, severity FROM public.system_health_checks WHERE tenant_id = $1 GROUP BY severity", [tenantId]));
  const criticalHealth = healthChecks.find((r) => r.severity === "critical") ? parseInt((healthChecks.find((r) => r.severity === "critical")?.count as string) ?? "0", 10) : 0;
  ...
  return c.json({
    ...
    infrastructure: {
      ...
      backup_success_rate: totalBackups > 0 ? Math.round((successfulBackups / totalBackups) * 100) : 0,
    },
    platform: {
...
executiveDashboardRoute.get("/trends", httpCache(120), async (c) => {
  ...
  const ticketTrends = safeRows(await query(
  ...
  const deviceStatusBreakdown = safeRows(await query(
  ...
  const backupTrends = safeRows(await query(
  ...
  const notificationTrends = safeRows(await query(
    `SELECT DATE(created_at) as date, COUNT(*) as count,
  ...
  const auditTrends = safeRows(await query(
    `SELECT DATE(created_at) as date, COUNT(*) as count
...
executiveDashboardRoute.get("/alerts", httpCache(30), async (c) => {
  ...
  const criticalTickets = safeRows(await query(
  ...
  const sslAlerts = safeRows(await query(
  ...
  // Capacity risks
  const capacityRisks = safeRows(await query(
  ...
  const failedBackups = safeRows(await query(
  ...
  const healthAlerts = safeRows(await query(
  ...
  const totalAlerts = criticalTickets.length + sslAlerts.length + capacityRisks.length + failedBackups.length + healthAlerts.length;
...
executiveDashboardRoute.get("/summary", httpCache(60), async (c) => {
  ...
  const topDevices = safeRows(await query(
  ...
  const recentActivity = safeRows(await query(
  ...
  const recentTickets = safeRows(await query(
    `SELECT id, subject, priority, status, created_at FROM public.tickets
  ...
  const recentBackups = safeRows(await query(
