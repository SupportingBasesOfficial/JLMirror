...
securityAuditRoute.use("/*", jwtAuth);
securityAuditRoute.use("/*", tenantContext);

const categorySchema = z.enum(["access_control", "encryption", "compliance", "vulnerability", "configuration", "network", "data_protection"]);
const severitySchema = z.enum(["info", "low", "medium", "high", "critical"]);
const checkTypeSchema = z.enum(["sql_query", "config_check", "ssl_check", "password_policy", "rls_check", "session_check", "custom"]);
...
securityAuditRoute.get("/rules", requirePermission("compliance:read"), async (c) => {
...
securityAuditRoute.post("/rules", requirePermission("compliance:write"), async (c) => {
  const user = c.get("user");
  ...
  const result = await query<{ id: string }>(
    ...
    [
      ...
      data.category, data.severity, data.check_type, data.check_query ?? null,
      JSON.stringify(data.check_config ?? {}), data.expected_result ?? null,
      data.remediation ?? null, data.is_active, user.sub,
    ],
  );
...
securityAuditRoute.put("/rules/:id", requirePermission("compliance:write"), async (c) => {
  ...
  const fieldMap: Record<string, string> = {
    name: "name", description: "description", category: "category", severity: "severity",
    check_type: "check_type", check_query: "check_query", expected_result: "expected_result",
    remediation: "remediation", is_active: "is_active",
  };
...
securityAuditRoute.delete("/rules/:id", requirePermission("compliance:write"), async (c) => {
  const ruleId = c.req.param("id");
  const user = c.get("user");
...
securityAuditRoute.post("/scan", requirePermission("compliance:write"), async (c) => {
  const user = c.get("user");
  ...
  if (!scanId) {
    return c.json({ error: { code: "CREATE_ERROR", message: "Erro ao criar scan" } }, 500);
  }
  ...
  try {
    ...
    for (const rule of rules) {
      if (rule.check_type === "sql_query" && rule.check_query) {
        try {
          ...
          const count = rows.length > 0 ? parseInt((rows[0].count as string) ?? String(rows.length), 10) : 0;
          ...
          if (count > expected) {
            ...
            const findingResult = await query<{ id: string }>(
              ...
              [
                tenantId, rule.id, scanId, rule.severity,
        ...
        try {
          ...
          if (rlsRows.length > 0) {
            const findingResult = await query<{ id: string }>(
              ...
              [
                tenantId, rule.id, scanId, rule.severity,
                JSON.stringify({ tables_without_rls: rlsRows.map((r) => r.relname) }),
    ...
    await query(
      `UPDATE public.security_audit_scans SET
       ...
       WHERE id = $9`,
      [rules.length, rules.length, totalFindings, criticalFindings, highFindings, mediumFindings, lowFindings, durationMs, scanId],
    );

    await query(
      "SELECT public.write_audit_log($1, NULL, 'audit.scan', 'security_audit_scans', NULL, $2, NULL, NULL)",
      [user.sub, JSON.stringify({ scan_id: scanId, findings: totalFindings, critical: criticalFindings })],
    );
...
securityAuditRoute.get("/findings", requirePermission("compliance:read"), httpCache(30), async (c) => {
  const user = c.get("user");
  ...
  if (status) { conditions.push(`f.status = $${paramIdx++}`); params.push(status); }
  if (severity) { conditions.push(`f.severity = $${paramIdx++}`); params.push(severity); }
  params.push(limit);
...
securityAuditRoute.post("/findings/:id/remediate", requirePermission("compliance:write"), async (c) => {
...
securityAuditRoute.post("/findings/:id/false-positive", requirePermission("compliance:write"), async (c) => {
  const findingId = c.req.param("id");
  const user = c.get("user");
...
securityAuditRoute.get("/scans", requirePermission("compliance:read"), async (c) => {
  const user = c.get("user");
...
securityAuditRoute.get("/summary", requirePermission("compliance:read"), httpCache(30), async (c) => {
  const user = c.get("user");
  ...
  return c.json({
    findings: findingsResult.data?.rows[0] ?? { open: "0", critical_open: "0", high_open: "0", medium_open: "0", low_open: "0", remediated: "0", false_positive: "0", total: "0" },
