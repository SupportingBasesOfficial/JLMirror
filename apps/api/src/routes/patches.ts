  ...
  async (c) => {
    ...
    await query(
      ...
      [
        user.sub,
        patchId,
        JSON.stringify({ patch_name: result.data.rows[0].patch_name }),
      ],
    );
...
patchRoute.post("/:id/reject", requirePermission("assets:write"), async (c) => {
  ...
  await query(
    ...
    [
      user.sub,
      patchId,
      JSON.stringify({ reason: body.reason ?? "Rejeitado manualmente" }),
    ],
  );
...
patchRoute.post(
  ...
  async (c) => {
    ...
    const requiresReboot =
      parseInt((rebootResult.data?.rows[0]?.count as string) ?? "0", 10) > 0;
    ...
    if (result.error || !result.data?.rows[0]) {
      return c.json(
        {
          error: { code: "CREATE_ERROR", message: "Erro ao criar deployment" },
        },
        500,
      );
    }
  ...
  async (c) => {
    ...
    await query(
      ...
      [
        user.sub,
        deploymentId,
        JSON.stringify({ successful: body.successful, failed: body.failed }),
      ],
    );
