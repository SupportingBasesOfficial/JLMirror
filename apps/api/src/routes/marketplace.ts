    ...
    const installResult = await query<{ id: string }>(
     ...
     RETURNING id`,
      [
        tenantId,
        appId,
        JSON.stringify(body.config ?? app.default_config ?? {}),
        user.sub,
      ],
    );
    ...
    await query(
      ...
      [
        ...
        JSON.stringify({
          app_slug: app.slug,
          app_name: app.name,
          install_id: installResult.data?.rows[0]?.id,
        }),
      ],
    );
