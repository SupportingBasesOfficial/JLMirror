...
predictionRoute.post(
  ...
  async (c) => {
      ...
      return c.json(
        {
          error: {
            code: "NO_CONFIG",
            message: "Nenhuma configuração de predição ativa para esta métrica",
          },
        },
        404,
      );
    ...
    if (result.predictedFailure) {
      ...
      return c.json({
        prediction_id: insertResult.data?.rows[0]?.id,
        ...result,
      });
    }
...
predictionRoute.get(
  ...
  async (c) => {
    ...
    const accuracy =
      total > 0 ? ((occurred + getCount(mitigatedResult)) / total) * 100 : 0;
