...
import { safeCount } from "./query-helpers.js";
  ...
  return cachedQuery<HealthScoreResult>(
    ...
    async () => {
      ...
      const deviceUptimePct =
        totalDevices > 0 ? (activeDevices / totalDevices) * 100 : 100;
      breakdown.push({
      ...
      breakdown.push({
        ...
        details: {
          ...
          failed: totalBackups - successfulBackups,
        },
      });

      // 3. Certificados SSL válidos (15%)
      const totalSsl = safeCount(
        await query(
          "SELECT COUNT(*) as count FROM public.ssl_certificates_with_status WHERE tenant_id = $1",
          [tenantId],
        ),
      );
      const validSsl = safeCount(
        await query(
      ...
      breakdown.push({
        ...
        details: {
          total: totalSsl,
          valid: validSsl,
          expired_or_expiring: totalSsl - validSsl,
        },
      });

      // 4. Health checks saudáveis (15%)
      const totalChecks = safeCount(
        await query(
          "SELECT COUNT(*) as count FROM public.system_health_checks WHERE tenant_id = $1 AND is_active = true",
          [tenantId],
        ),
      );
      const healthyChecks = safeCount(
        await query(
      ...
      const incidentPenalty =
        criticalIncidents * 20 + (activeIncidents - criticalIncidents) * 5;
      ...
      const complianceScore =
        totalScans > 0 ? (passedScans / totalScans) * 100 : 100;
      breakdown.push({
